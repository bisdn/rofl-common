 ...
 * its associated port on the switch for reaching this host on a given datapath
 * element.
 * Class cfibentry defines the container for an active FIB entry. Class
 * cfibtable
 * defines two groups of methods:
 *
 * 1. Static methods on CRUD operations for instances of class cfibtable
 *
 * 2. Methods on CRUD operations for instances of class cfibentry in a cfibtable
 * instance
 *
 * @see cfibentry
 * @see cfibentry_env
...
public:
  ...
  /**
   * @brief	Returns reference to new or existing and resetted cfibtable
   * instance.
   *
  ...
  /**
   * @brief	Returns reference to existing cfibtable instance or creates a
   * new
   * empty one.
   *
  ...
  /**
   * @brief	Returns const reference to existing cfibtable instance or throws
   * exception.
   *
  ...
  static bool has_fib(const rofl::cdptid &dptid) {
    return (
        not(cfibtable::fibtables.find(dptid) == cfibtable::fibtables.end()));
  };
...
public:
   ...
   */
  void clear() {
    for (std::map<rofl::caddress_ll, cfibentry *>::iterator it = ftable.begin();
         it != ftable.end(); ++it) {
      delete it->second;
    }
    ftable.clear();
  ...
  /**
   * @brief	Returns reference to empty cfibentry instance for given hardware
   * address.
   *
  ...
  cfibentry &add_fib_entry(const rofl::caddress_ll &hwaddr, uint32_t portno) {
    if (hwaddr.is_multicast() || hwaddr.is_null()) {
      throw exceptions::eFibInval(
          "cfibtable::add_fib_entry() hwaddr validation failed");
    }
  ...
  /**
   * @brief	Returns reference to existing cfibentry instance for given
   * hardware address.
   *
  ...
  cfibentry &set_fib_entry(const rofl::caddress_ll &hwaddr, uint32_t portno) {
    if (hwaddr.is_multicast() || hwaddr.is_null()) {
      throw exceptions::eFibInval(
          "cfibtable::set_fib_entry() hwaddr validation failed");
    }
    if (ftable.find(hwaddr) == ftable.end()) {
      ftable[hwaddr] = new cfibentry(this, hwaddr, portno);
    }
  ...
  cfibentry &set_fib_entry(const rofl::caddress_ll &hwaddr) {
    ...
    if (ftable.find(hwaddr) == ftable.end()) {
      throw exceptions::eFibNotFound(
          "cfibtable::set_fib_entry() hwaddr not found");
    }
    return *(ftable[hwaddr]);
  };

  /**
   * @brief	Returns const reference to existing cfibentry instance for given
   * hardware address.
   *
  ...
  const cfibentry &get_fib_entry(const rofl::caddress_ll &hwaddr) const {
    if (hwaddr.is_multicast() || hwaddr.is_null()) {
      throw exceptions::eFibInval(
          "cfibtable::get_fib_entry() hwaddr validation failed");
    }
    if (ftable.find(hwaddr) == ftable.end()) {
      throw exceptions::eFibNotFound(
          "cfibtable::set_fib_entry() hwaddr not found");
...
private:
   ...
   */
  cfibtable(const rofl::cdptid &dptid) : dptid(dptid) {
...
private:
   ...
   */
  virtual void fib_timer_expired(const rofl::caddress_ll &hwaddr) {
    drop_fib_entry(hwaddr);
  };
...
public:
   ...
   *
   */
  friend std::ostream &operator<<(std::ostream &os, cfibtable const &fib) {
    try {
#if 0
			os << "<cfibtable dpid: "
    ...
    for (std::map<rofl::caddress_ll, cfibentry *>::const_iterator it =
             fib.ftable.begin();
         it != fib.ftable.end(); ++it) {
      os << *(it->second);
    }
    return os;
  };
