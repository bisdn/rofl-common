...
void cofmeterbands_test::setUp() {}

void cofmeterbands_test::tearDown() {}

void cofmeterbands_test::testDefaultConstructor() {
...
void cofmeterbands_test::testCopyConstructor() {
...
void cofmeterbands_test::testPackUnpack() {
  uint8_t of_version = rofl::openflow13::OFP_VERSION;
  ...
  CPPUNIT_ASSERT(0xe1e2e3e4 ==
                 clone.get_meter_band_experimenter(0).get_exp_id());
  for (unsigned int i = 0; i < 6; i++) {
    CPPUNIT_ASSERT(i == clone.get_meter_band_experimenter().get_body()[i]);
  }
}

void cofmeterbands_test::testInvalidLength() {
  uint8_t of_version = rofl::openflow13::OFP_VERSION;
