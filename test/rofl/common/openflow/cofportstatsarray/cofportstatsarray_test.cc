...
void cofportstatsarray_test::setUp() {}

void cofportstatsarray_test::tearDown() {}

void cofportstatsarray_test::testDefaultConstructor() {
  rofl::openflow::cofportstatsarray array;
  CPPUNIT_ASSERT(rofl::openflow::OFP_VERSION_UNKNOWN == array.get_version());
}

void cofportstatsarray_test::testCopyConstructor() {
...
void cofportstatsarray_test::testOperatorPlus() {
  std::vector<rofl::openflow::cofportstatsarray> array;
  array.push_back(
      rofl::openflow::cofportstatsarray(rofl::openflow13::OFP_VERSION));
  array.push_back(
      rofl::openflow::cofportstatsarray(rofl::openflow13::OFP_VERSION));
  ...
  std::cerr << "merge.set_port_stats(0) == array[0].get_port_stats(0): "
            << (merge.set_port_stats(0) == array[0].get_port_stats(0))
            << std::endl;
  std::cerr << "merge.set_port_stats(1) == array[0].get_port_stats(1): "
            << (merge.set_port_stats(1) == array[0].get_port_stats(1))
            << std::endl;
  std::cerr << "merge.set_port_stats(2) == array[1].get_port_stats(2): "
            << (merge.set_port_stats(2) == array[1].get_port_stats(2))
            << std::endl;
#endif
...
void cofportstatsarray_test::testPackUnpack() {
  /*
   * unpack
   */
  rofl::cmemory mem(sizeof(struct rofl::openflow13::ofp_port_stats));
  struct rofl::openflow13::ofp_port_stats *port =
      (struct rofl::openflow13::ofp_port_stats *)mem.somem();
  ...
  CPPUNIT_ASSERT(array.get_port_stats(8).get_rx_packets() ==
                 0x1112131415161718);
  CPPUNIT_ASSERT(array.get_port_stats(8).get_tx_packets() ==
                 0x2122232425262728);
  ...
  CPPUNIT_ASSERT(array.get_port_stats(8).get_rx_dropped() ==
                 0x5152535455565758);
  CPPUNIT_ASSERT(array.get_port_stats(8).get_tx_dropped() ==
                 0x6162636465666768);
  ...
  CPPUNIT_ASSERT(array.get_port_stats(8).get_rx_frame_err() ==
                 0x9192939495969798);
  CPPUNIT_ASSERT(array.get_port_stats(8).get_rx_over_err() ==
                 0xa1a2a3a4a5a6a7a8);
  CPPUNIT_ASSERT(array.get_port_stats(8).get_rx_crc_err() ==
                 0xb1b2b3b4b5b6b7b8);
  CPPUNIT_ASSERT(array.get_port_stats(8).get_collisions() ==
                 0xc1c2c3c4c5c6c7c8);
...
void cofportstatsarray_test::testAddDropSetGetHas() {
  rofl::openflow::cofportstatsarray array(rofl::openflow13::OFP_VERSION);
