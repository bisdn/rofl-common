...
void coxmatchtest::setUp() {
  oxm_id_exp = 0xffffa3a4;
  exp_id = 0x01020304;
}

void coxmatchtest::tearDown() {}

void coxmatchtest::test() {
  ...
  CPPUNIT_ASSERT(oxm.get_oxm_type() ==
                 (uint32_t)((rofl::openflow::OFPXMC_OPENFLOW_BASIC << 16) |
                            (rofl::openflow::OFPXMT_OFB_ETH_TYPE << 9)));
}

void coxmatchtest::test_eth_dst() {
...
void coxmatchtest::test_eth_dst_mask() {
  ...
  CPPUNIT_ASSERT(oxm.length() ==
                 sizeof(struct rofl::openflow::ofp_oxm_hdr) + 2 * OFP_ETH_ALEN);
  CPPUNIT_ASSERT(oxm.get_oxm_class() == rofl::openflow::OFPXMC_OPENFLOW_BASIC);
  CPPUNIT_ASSERT(oxm.get_oxm_field() == rofl::openflow::OFPXMT_OFB_ETH_DST);
...
void coxmatchtest::test1ByteHasMask() {
  uint32_t oxm_id = 0xa1a2a3a4 | HAS_MASK_FLAG;
...
void coxmatchtest::test2BytesHasMask() {
  uint32_t oxm_id = 0xa1a2a3a4 | HAS_MASK_FLAG;
...
void coxmatchtest::test3BytesHasMask() {
  uint32_t oxm_id = 0xa1a2a3a4 | HAS_MASK_FLAG;
...
void coxmatchtest::test4BytesHasMask() {
  uint32_t oxm_id = 0xa1a2a3a4 | HAS_MASK_FLAG;
...
void coxmatchtest::test6Bytes() {
  ...
  CPPUNIT_ASSERT(mem == test);
  CPPUNIT_ASSERT(oxm.get_u48value_as_lladdr() ==
                 rofl::cmacaddr("b1:b2:b3:b4:b5:b6"));
  CPPUNIT_ASSERT(oxm.get_u48mask_as_lladdr() ==
                 rofl::cmacaddr("ff:ff:ff:ff:ff:ff"));
  CPPUNIT_ASSERT(oxm.get_u48masked_value_as_lladdr() ==
                 rofl::cmacaddr("b1:b2:b3:b4:b5:b6"));
...
void coxmatchtest::test6BytesHasMask() {
  uint32_t oxm_id = 0xa1a2a3a4 | HAS_MASK_FLAG;
  ...
  CPPUNIT_ASSERT(oxm.get_u48value_as_lladdr() ==
                 rofl::cmacaddr("d1:d2:d3:d4:d5:d6"));
  CPPUNIT_ASSERT(oxm.get_u48mask_as_lladdr() ==
                 rofl::cmacaddr("e1:e2:e3:e4:e5:e6"));
  CPPUNIT_ASSERT(oxm.get_u48masked_value_as_lladdr() ==
                 (rofl::cmacaddr("d1:d2:d3:d4:d5:d6") &
                  rofl::cmacaddr("e1:e2:e3:e4:e5:e6")));
}

void coxmatchtest::test8Bytes() {
  uint32_t oxm_id = 0xa1a2a3a4 & ~HAS_MASK_FLAG;
...
void coxmatchtest::test8BytesHasMask() {
  uint32_t oxm_id = 0xa1a2a3a4 | HAS_MASK_FLAG;

  rofl::openflow::coxmatch_64 oxm(oxm_id, (uint64_t)0xb1b2b3b4b5b6b7b8,
                                  (uint64_t)0xc1c2c3c4c5c6c7c8);
  ...
  CPPUNIT_ASSERT(oxm.get_u64masked_value() ==
                 (0xb1b2b3b4b5b6b7b8 & 0xc1c2c3c4c5c6c7c8));
  ...
  CPPUNIT_ASSERT(oxm.get_u64masked_value() ==
                 (0xd1d2d3d4d5d6d7d8 & 0xe1e2e3e4e5e6e7e8));
}

void coxmatchtest::testOxmIPv4Src() {
...
void coxmatchtest::testOxmIPv4Dst() {
  // TODO
}

void coxmatchtest::testOxmIPv6Src() {
...
void coxmatchtest::testOxmIPv6Dst() {
  // TODO
  // exit(0);
}

void coxmatchtest::testExp8() {
  uint16_t oxm_class = 0xffff;
  uint8_t oxm_field = 0x74; // must be < 128 (0x80)
  uint32_t exp_id = 0xe1e2e3e4;
  uint32_t oxm_id =
      (((uint32_t)oxm_class) << 16) | (((uint16_t)oxm_field) << 9);
  uint8_t value = 0xa1;
  uint8_t mask = 0xb2;
  ...
  std::cerr << "clone.get_oxm_type(): 0x" << std::hex
            << (unsigned int)clone.get_oxm_type() << std::endl;
  std::cerr << "clone.get_oxm_id(): 0x" << std::hex
            << (unsigned int)clone.get_oxm_id() << std::endl;
  ...
  CPPUNIT_ASSERT(clone.get_oxm_length() ==
                 sizeof(uint32_t) + 2 * sizeof(uint8_t));
}

void coxmatchtest::testExp16() {
  uint16_t oxm_class = 0xffff;
  uint8_t oxm_field = 0x74; // must be < 128 (0x80)
  uint32_t exp_id = 0xe1e2e3e4;
  uint32_t oxm_id =
...
void coxmatchtest::testExp48() {
  ...
  CPPUNIT_ASSERT(clone.length() == oxm.length());
  CPPUNIT_ASSERT(clone.get_oxm_length() ==
                 sizeof(uint32_t) + 6 * sizeof(uint8_t));
  ...
  CPPUNIT_ASSERT(clone.length() == oxm.length());
  CPPUNIT_ASSERT(clone.get_oxm_length() ==
                 sizeof(uint32_t) + 2 * 6 * sizeof(uint8_t));
}

void coxmatchtest::testExp64() {
  uint16_t oxm_class = 0xffff;
  uint8_t oxm_field = 0x74; // must be < 128 (0x80)
  uint32_t exp_id = 0xe1e2e3e4;
  uint32_t oxm_id =
