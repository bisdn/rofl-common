...
void cofmeterbandstatsarray_test::setUp() {}

void cofmeterbandstatsarray_test::tearDown() {}

void cofmeterbandstatsarray_test::testDefaultConstructor() {
...
void cofmeterbandstatsarray_test::testCopyConstructor() {
  ...
  for (unsigned int i = 0; i < 4; i++) {
    CPPUNIT_ASSERT(mbs2.get_mbstats(i).get_packet_band_count() ==
                   packet_band_count);
    CPPUNIT_ASSERT(mbs2.set_mbstats(i).get_byte_band_count() ==
                   byte_band_count);
  }
}

void cofmeterbandstatsarray_test::testPackUnpack() {
  uint8_t of_version = rofl::openflow13::OFP_VERSION;
  uint64_t packet_band_count = 0xa1a2a3a4a5a6a7a8;
  uint64_t byte_band_count = 0xb1b2b3b4b5b6b7b8;
