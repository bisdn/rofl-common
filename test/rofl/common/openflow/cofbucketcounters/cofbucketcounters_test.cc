...
void cofbucket_counters_test::setUp() {}

void cofbucket_counters_test::tearDown() {}

void cofbucket_counters_test::testDefaultConstructor() {
  rofl::openflow::cofbucket_counters bcs;
  CPPUNIT_ASSERT(rofl::openflow::OFP_VERSION_UNKNOWN == bcs.get_version());
}

void cofbucket_counters_test::testCopyConstructor() {
  rofl::openflow::cofbucket_counters bcs(rofl::openflow13::OFP_VERSION);

  uint32_t bucket_counter_id = 0;

  bcs.set_bucket_counter(bucket_counter_id)
  ...
  CPPUNIT_ASSERT(bcs.length() ==
                 4 * sizeof(struct rofl::openflow13::ofp_bucket_counter));
  CPPUNIT_ASSERT(mbcs == mclone);
}

void cofbucket_counters_test::testPackUnpack() {
  uint64_t mem[8];
...
#ifdef DEBUG
    std::cerr << "mbcs[" << (int)(8 * i + 7) << "]=" << (int)mbcs[8 * i + 7]
              << " i=" << i << std::endl;
#endif
...
void cofbucket_counters_test::testAddDropSetGetHas() {
  rofl::openflow::cofbucket_counters bcs(rofl::openflow13::OFP_VERSION);
