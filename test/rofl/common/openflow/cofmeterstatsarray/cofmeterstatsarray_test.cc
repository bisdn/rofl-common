...
void cofmeterstatsarray_test::setUp() {}

void cofmeterstatsarray_test::tearDown() {}

void cofmeterstatsarray_test::testDefaultConstructor() {
...
void cofmeterstatsarray_test::testCopyConstructor() {
  ...
  for (unsigned int i = 0; i < num_of_mstats; i++) {
    ...
    CPPUNIT_ASSERT(mstats2.get_meter_stats(i).get_packet_in_count() ==
                   packet_in_count);
    CPPUNIT_ASSERT(mstats2.get_meter_stats(i).get_byte_in_count() ==
                   byte_in_count);
    CPPUNIT_ASSERT(mstats2.get_meter_stats(i).get_duration_sec() ==
                   duration_sec);
    CPPUNIT_ASSERT(mstats2.get_meter_stats(i).get_duration_nsec() ==
                   duration_nsec);
  }
...
void cofmeterstatsarray_test::testPackUnpack() {
  unsigned int num_of_mstats = 5;
  uint32_t meter_id = 0xa1a2a3a4;
  uint32_t flow_count = 0xb1b2b3b4;
  uint64_t packet_in_count = 0xc1c2c3c4c5c6c7c8;
  ...
  for (unsigned int i = 0; i < num_of_mstats; i++) {
    ...
    for (unsigned int j = 0; j < 3; j++) {
      mstats1.set_meter_stats(i)
          .set_meter_band_stats()
          .set_mbstats(j)
          .set_packet_band_count(packet_band_count);
      mstats1.set_meter_stats(i)
          .set_meter_band_stats()
          .set_mbstats(j)
          .set_byte_band_count(byte_band_count);
    }
  }
  ...
  for (unsigned int i = 0; i < num_of_mstats; i++) {
    ...
    CPPUNIT_ASSERT(mstats2.get_meter_stats(i).get_duration_nsec() ==
                   duration_nsec);
    for (unsigned int j = 0; j < 3; j++) {
      CPPUNIT_ASSERT(mstats1.get_meter_stats(i)
                         .get_meter_band_stats()
                         .get_mbstats(j)
                         .get_packet_band_count() == packet_band_count);
      CPPUNIT_ASSERT(mstats1.get_meter_stats(i)
                         .get_meter_band_stats()
                         .get_mbstats(j)
                         .get_byte_band_count() == byte_band_count);
    }
  }
...
void cofmeterstatsarray_test::testOperatorPlus() {
  unsigned int num_of_mstats = 5;
  uint32_t meter_id = 0xa1a2a3a4;
  uint32_t flow_count = 0xb1b2b3b4;
  uint64_t packet_in_count = 0xc1c2c3c4c5c6c7c8;
  ...
  for (unsigned int i = 0; i < 2 * num_of_mstats; i++) {
    ...
    CPPUNIT_ASSERT(mstats3.get_meter_stats(i).get_packet_in_count() ==
                   packet_in_count);
    CPPUNIT_ASSERT(mstats3.get_meter_stats(i).get_byte_in_count() ==
                   byte_in_count);
    CPPUNIT_ASSERT(mstats3.get_meter_stats(i).get_duration_sec() ==
                   duration_sec);
    CPPUNIT_ASSERT(mstats3.get_meter_stats(i).get_duration_nsec() ==
                   duration_nsec);
  }
}

void cofmeterstatsarray_test::testAddDropSetGetHas() {
