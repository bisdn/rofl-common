...
void cofmeterband_test::setUp() {}

void cofmeterband_test::tearDown() {}

void cofmeterband_test::testDefaultConstructor() {
  rofl::openflow::cofmeter_band meterband;
  CPPUNIT_ASSERT(rofl::openflow::OFP_VERSION_UNKNOWN ==
                 meterband.get_version());
  CPPUNIT_ASSERT(0 == meterband.get_type());
  CPPUNIT_ASSERT(sizeof(struct rofl::openflow13::ofp_meter_band_header) ==
                 meterband.get_length());
  CPPUNIT_ASSERT(0 == meterband.get_rate());
  CPPUNIT_ASSERT(0 == meterband.get_burst_size());
}

void cofmeterband_test::testCopyConstructor() {
...
void cofmeterband_test::testPackUnpack() {
  uint8_t of_version = rofl::openflow13::OFP_VERSION;
  uint16_t type = 0xa1a2;
  uint32_t rate = 0xc1c2c3c4;
  uint32_t burst_size = 0xd1d2d3d4;
  ...
  uint16_t len =
      sizeof(struct rofl::openflow13::ofp_meter_band_header) + body.memlen();
...
void cofmeterband_test::testInvalidLength() {
  uint8_t of_version = rofl::openflow13::OFP_VERSION;
...
void cofmeterband_test::testDscpRemark() {
  uint8_t of_version = rofl::openflow13::OFP_VERSION;
  uint16_t type = 0xa1a2;
  uint32_t rate = 0xc1c2c3c4;
  uint32_t burst_size = 0xd1d2d3d4;
...
void cofmeterband_test::testExperimenter() {
  uint8_t of_version = rofl::openflow13::OFP_VERSION;
  uint16_t type = 0xa1a2;
  uint32_t rate = 0xc1c2c3c4;
  uint32_t burst_size = 0xd1d2d3d4;
