...
void cofflowmod_test::setUp() {}

void cofflowmod_test::tearDown() {}

void cofflowmod_test::testFlowMod10() {
  rofl::cindex index(0);
...
void cofflowmod_test::testFlowMod13() {
  rofl::cindex index(0);
  ...
  CPPUNIT_ASSERT(clone.get_match().get_eth_type() == eth_type);
  CPPUNIT_ASSERT(
      clone.get_instructions().get_inst_goto_table().get_table_id() ==
      (table_id + 1));
}
