...
void cofmeterconfigarray_test::setUp() {}

void cofmeterconfigarray_test::tearDown() {}

void cofmeterconfigarray_test::testDefaultConstructor() {
...
void cofmeterconfigarray_test::testCopyConstructor() {
...
void cofmeterconfigarray_test::testPackUnpack() {
  ...
  for (unsigned int i = 0; i < num_of_mconfigs; i++) {
    ...
    for (unsigned int j = 0; j < num_of_meter_configs; j++) {
      ...
      mconfig1.set_meter_config(i)
          .set_meter_bands()
          .set_meter_band_dscp_remark(j)
  ...
  for (unsigned int i = 0; i < num_of_mconfigs; i++) {
    ...
    CPPUNIT_ASSERT(mconfig2.get_meter_config(i).get_meter_id() == meter_id);
    CPPUNIT_ASSERT(
        mconfig2.get_meter_config(i).get_meter_bands().get_mbs_drop().size() ==
        num_of_meter_configs);
    CPPUNIT_ASSERT(mconfig2.get_meter_config(i)
                       .get_meter_bands()
                       .get_mbs_dscp_remark()
                       .size() == num_of_meter_configs);
    for (unsigned int j = 0; j < num_of_meter_configs; j++) {
      CPPUNIT_ASSERT(mconfig2.get_meter_config(i)
                         .get_meter_bands()
                         .get_meter_band_dscp_remark(j)
                         .get_prec_level() == prec_level);
    }
  }
...
void cofmeterconfigarray_test::testOperatorPlus() {
  unsigned int num_of_mconfigs = 2;
  unsigned int num_of_meter_configs = 3;
  uint16_t flags = 0xa1a2;
  uint32_t meter_id = 0xb1b2b3b4;
  ...
  for (unsigned int i = 0; i < 2 * num_of_mconfigs; i++) {
    ...
    CPPUNIT_ASSERT(
        mconfig3.get_meter_config(i).get_meter_bands().get_mbs_drop().size() ==
        num_of_meter_configs);
    CPPUNIT_ASSERT(mconfig3.get_meter_config(i)
                       .get_meter_bands()
                       .get_mbs_dscp_remark()
                       .size() == num_of_meter_configs);
    for (unsigned int j = 0; j < num_of_meter_configs; j++) {
...
void cofmeterconfigarray_test::testAddDropSetGetHas() {
