...
void cofpacketqueues_test::setUp() {}

void cofpacketqueues_test::tearDown() {}

void cofpacketqueues_test::testPacketQueues() {
  ...
  for (unsigned int i = 0; i < 2; i++) {
    for (unsigned int j = 0; j < 2; j++) {
      queues.set_pqueue(port_no + i, queue_id + j)
          .set_queue_props()
          .set_queue_prop_min_rate()
          .set_min_rate(min_rate);
      queues.set_pqueue(port_no + i, queue_id + j)
          .set_queue_props()
  ...
  for (unsigned int i = 0; i < 2; i++) {
    for (unsigned int j = 0; j < 2; j++) {
      CPPUNIT_ASSERT(
          clone.get_pqueue(port_no + i, queue_id + j).get_port_no() ==
          port_no + i);
      CPPUNIT_ASSERT(
          clone.get_pqueue(port_no + i, queue_id + j).get_queue_id() ==
          queue_id + j);
      CPPUNIT_ASSERT(clone.get_pqueue(port_no + i, queue_id + j)
                         .get_queue_props()
                         .get_queue_prop_min_rate()
                         .get_min_rate() ==
                     queues.get_pqueue(port_no + i, queue_id + j)
                         .get_queue_props()
                         .get_queue_prop_min_rate()
                         .get_min_rate());
      CPPUNIT_ASSERT(clone.get_pqueue(port_no + i, queue_id + j)
                         .get_queue_props()
                         .get_queue_prop_max_rate()
                         .get_max_rate() ==
                     queues.get_pqueue(port_no + i, queue_id + j)
                         .get_queue_props()
                         .get_queue_prop_max_rate()
