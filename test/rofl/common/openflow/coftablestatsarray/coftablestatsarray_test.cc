...
void coftablestatsarray_test::setUp() {}

void coftablestatsarray_test::tearDown() {}

void coftablestatsarray_test::testDefaultConstructor() {
  rofl::openflow::coftablestatsarray array;
  CPPUNIT_ASSERT(rofl::openflow::OFP_VERSION_UNKNOWN == array.get_version());
}

void coftablestatsarray_test::testCopyConstructor() {
...
void coftablestatsarray_test::testOperatorPlus() {
  std::vector<rofl::openflow::coftablestatsarray> array;
  array.push_back(
      rofl::openflow::coftablestatsarray(rofl::openflow13::OFP_VERSION));
  array.push_back(
      rofl::openflow::coftablestatsarray(rofl::openflow13::OFP_VERSION));
  ...
  std::cerr << "merge.set_table_stats(0) == array[0].get_table_stats(0): "
            << (merge.set_table_stats(0) == array[0].get_table_stats(0))
            << std::endl;
  std::cerr << "merge.set_table_stats(1) == array[0].get_table_stats(1): "
            << (merge.set_table_stats(1) == array[0].get_table_stats(1))
            << std::endl;
  std::cerr << "merge.set_table_stats(4) == array[1].get_table_stats(4): "
            << (merge.set_table_stats(4) == array[1].get_table_stats(4))
            << std::endl;
#endif
...
void coftablestatsarray_test::testPackUnpack() {
  /*
   * unpack
   */
  rofl::cmemory mem(sizeof(struct rofl::openflow13::ofp_table_stats));
  struct rofl::openflow13::ofp_table_stats *table =
      (struct rofl::openflow13::ofp_table_stats *)mem.somem();
  ...
  CPPUNIT_ASSERT(array.get_table_stats(8).get_lookup_count() ==
                 0x2122232425262728);
  CPPUNIT_ASSERT(array.get_table_stats(8).get_matched_count() ==
                 0x3132333435363738);
...
void coftablestatsarray_test::testAddDropSetGetHas() {
  rofl::openflow::coftablestatsarray array(rofl::openflow13::OFP_VERSION);
