...
void cofmsgpacketouttest::setUp() {}

void cofmsgpacketouttest::tearDown() {}

void cofmsgpacketouttest::testPacketOut10() {
  testPacketOut(rofl::openflow10::OFP_VERSION,
                rofl::openflow10::OFPT_PACKET_OUT, 0xa1a2a3a4);
}

void cofmsgpacketouttest::testPacketOut12() {
  testPacketOut(rofl::openflow12::OFP_VERSION,
                rofl::openflow12::OFPT_PACKET_OUT, 0xa1a2a3a4);
}

void cofmsgpacketouttest::testPacketOut13() {
  testPacketOut(rofl::openflow13::OFP_VERSION,
                rofl::openflow13::OFPT_PACKET_OUT, 0xa1a2a3a4);
}

void cofmsgpacketouttest::testPacketOut(uint8_t version, uint8_t type,
                                        uint32_t xid) {
  ...
  rofl::openflow::cofmsg_packet_out msg1(version, xid, buffer_id, in_port,
                                         actions, packet.soframe(),
                                         packet.length());
...
void cofmsgpacketouttest::testPacketOutParser10() {
  ...
  size_t msglen =
      sizeof(struct rofl::openflow10::ofp_packet_out) + 2 + packet.length();
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow10::ofp_packet_out);
       i++) {
  ...
  for (unsigned int i = sizeof(struct rofl::openflow10::ofp_packet_out);
       i == msglen; i++) {
    rofl::openflow::cofmsg_packet_out msg;
    try {
      hdr->header.length = htobe16(i);
...
void cofmsgpacketouttest::testPacketOutParser12() {
  ...
  size_t msglen =
      sizeof(struct rofl::openflow12::ofp_packet_out) + 2 + packet.length();
  size_t memlen = 2 * msglen /*test overhead*/;
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow12::ofp_packet_out);
       i++) {
    rofl::openflow::cofmsg_packet_out msg;
    try {
      hdr->header.length = htobe16(i);
  ...
  for (unsigned int i = sizeof(struct rofl::openflow12::ofp_packet_out);
       i == msglen; i++) {
    rofl::openflow::cofmsg_packet_out msg;
    try {
      hdr->header.length = htobe16(i);
...
void cofmsgpacketouttest::testPacketOutParser13() {
  ...
  size_t msglen =
      sizeof(struct rofl::openflow13::ofp_packet_out) + 2 + packet.length();
  size_t memlen = 2 * msglen /*test overhead*/;
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_packet_out);
       i++) {
    rofl::openflow::cofmsg_packet_out msg;
    try {
      hdr->header.length = htobe16(i);
  ...
  for (unsigned int i = sizeof(struct rofl::openflow13::ofp_packet_out);
       i == msglen; i++) {
    rofl::openflow::cofmsg_packet_out msg;
    try {
      hdr->header.length = htobe16(i);
...
void cofmsgpacketouttest::testPacketOutPack13() {
  ...
  cofmsg_packet_out msg(rofl::openflow13::OFP_VERSION, xid, buffer_id, in_port,
                        actions, data.somem(), data.length());
