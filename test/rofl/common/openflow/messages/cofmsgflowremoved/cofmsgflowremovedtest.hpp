...
class cofmsgflowremovedtest : public CppUnit::TestFixture {
...
private:
  void testFlowRemoved(uint8_t version, uint8_t type, uint32_t xid);
  void testFlowRemovedParser(uint8_t version, uint8_t type, uint32_t xid);
};
