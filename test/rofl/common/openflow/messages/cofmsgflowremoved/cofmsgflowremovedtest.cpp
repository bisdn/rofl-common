...
void cofmsgflowremovedtest::setUp() {}

void cofmsgflowremovedtest::tearDown() {}

void cofmsgflowremovedtest::testFlowRemoved10() {
  testFlowRemoved(rofl::openflow10::OFP_VERSION,
                  rofl::openflow10::OFPT_FLOW_REMOVED, 0xa1a2a3a4);
}

void cofmsgflowremovedtest::testFlowRemoved12() {
  testFlowRemoved(rofl::openflow12::OFP_VERSION,
                  rofl::openflow12::OFPT_FLOW_REMOVED, 0xa1a2a3a4);
}

void cofmsgflowremovedtest::testFlowRemoved13() {
  testFlowRemoved(rofl::openflow13::OFP_VERSION,
                  rofl::openflow13::OFPT_FLOW_REMOVED, 0xa1a2a3a4);
}

void cofmsgflowremovedtest::testFlowRemoved(uint8_t version, uint8_t type,
                                            uint32_t xid) {
  ...
  rofl::openflow::cofmsg_flow_removed msg1(
      version, xid, cookie, priority, reason, table_id, duration_sec,
...
void cofmsgflowremovedtest::testFlowRemovedParser10() {
  ...
  for (unsigned int i = 1;
       i < sizeof(struct rofl::openflow10::ofp_flow_removed); i++) {
  ...
  for (unsigned int i = sizeof(struct rofl::openflow10::ofp_flow_removed);
       i == msglen; i++) {
    rofl::openflow::cofmsg_flow_removed msg;
    try {
      hdr->header.length = htobe16(i);
...
void cofmsgflowremovedtest::testFlowRemovedParser12() {
  ...
  for (unsigned int i = 1;
       i < sizeof(struct rofl::openflow12::ofp_flow_removed); i++) {
    rofl::openflow::cofmsg_flow_removed msg;
    try {
      hdr->header.length = htobe16(i);
      msg.unpack(mem.somem(), i);
  ...
  for (unsigned int i = sizeof(struct rofl::openflow12::ofp_flow_removed);
       i == msglen; i++) {
    rofl::openflow::cofmsg_flow_removed msg;
    try {
      hdr->header.length = htobe16(i);
...
void cofmsgflowremovedtest::testFlowRemovedParser13() {
  ...
  for (unsigned int i = 1;
       i < sizeof(struct rofl::openflow13::ofp_flow_removed); i++) {
    rofl::openflow::cofmsg_flow_removed msg;
    try {
      hdr->header.length = htobe16(i);
      msg.unpack(mem.somem(), i);
  ...
  for (unsigned int i = sizeof(struct rofl::openflow13::ofp_flow_removed);
       i == msglen; i++) {
    rofl::openflow::cofmsg_flow_removed msg;
    try {
      hdr->header.length = htobe16(i);
