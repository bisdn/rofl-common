...
class cofmsgerrortest : public CppUnit::TestFixture {
...
private:
  void testError(uint8_t version, uint8_t type, uint32_t xid);
  void testErrorParser(uint8_t version, uint8_t type, uint32_t xid);
};
