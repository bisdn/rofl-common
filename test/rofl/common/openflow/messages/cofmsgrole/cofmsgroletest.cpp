...
void cofmsgroletest::setUp() {}

void cofmsgroletest::tearDown() {}

void cofmsgroletest::testRequest12() {
  testRequest(rofl::openflow12::OFP_VERSION,
              rofl::openflow12::OFPT_ROLE_REQUEST, 0xa1a2a3a4);
}

void cofmsgroletest::testRequest13() {
  testRequest(rofl::openflow13::OFP_VERSION,
              rofl::openflow13::OFPT_ROLE_REQUEST, 0xa1a2a3a4);
}

void cofmsgroletest::testRequest(uint8_t version, uint8_t type, uint32_t xid) {
...
void cofmsgroletest::testReply12() {
  testReply(rofl::openflow12::OFP_VERSION, rofl::openflow12::OFPT_ROLE_REPLY,
            0xa1a2a3a4);
}

void cofmsgroletest::testReply13() {
  testReply(rofl::openflow13::OFP_VERSION, rofl::openflow13::OFPT_ROLE_REPLY,
            0xa1a2a3a4);
}

void cofmsgroletest::testReply(uint8_t version, uint8_t type, uint32_t xid) {
  rofl::openflow::cofrole ofrole(version);
  uint32_t role = 0x91929394;
  uint64_t generation_id = 0x8182838485868788;
  ofrole.set_role(role);
...
void cofmsgroletest::testRequestParser12() {
  testRequestParser(rofl::openflow12::OFP_VERSION,
                    rofl::openflow12::OFPT_ROLE_REQUEST, 0xa1a2a3a4);
}

void cofmsgroletest::testRequestParser13() {
  testRequestParser(rofl::openflow13::OFP_VERSION,
                    rofl::openflow13::OFPT_ROLE_REQUEST, 0xa1a2a3a4);
}

void cofmsgroletest::testRequestParser(uint8_t version, uint8_t type,
                                       uint32_t xid) {
  uint32_t role = 0x91929394;
  uint64_t generation_id = 0x8182838485868788;
  ...
  for (unsigned int i = 1;
       i < sizeof(struct rofl::openflow13::ofp_role_request); i++) {
  ...
  for (unsigned int i = sizeof(struct rofl::openflow13::ofp_role_request);
       i == msglen; i++) {
    rofl::openflow::cofmsg_role_request msg;
    try {
      hdr->header.length = htobe16(i);
...
void cofmsgroletest::testReplyParser12() {
  testReplyParser(rofl::openflow12::OFP_VERSION,
                  rofl::openflow12::OFPT_ROLE_REPLY, 0xa1a2a3a4);
}

void cofmsgroletest::testReplyParser13() {
  testReplyParser(rofl::openflow12::OFP_VERSION,
                  rofl::openflow12::OFPT_ROLE_REPLY, 0xa1a2a3a4);
}

void cofmsgroletest::testReplyParser(uint8_t version, uint8_t type,
                                     uint32_t xid) {
  uint32_t role = 0x91929394;
  uint64_t generation_id = 0x8182838485868788;
  size_t msglen = sizeof(struct rofl::openflow13::ofp_role_request);
