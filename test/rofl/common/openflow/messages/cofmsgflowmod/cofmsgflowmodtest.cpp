...
void cofmsgflowmodtest::setUp() {}

void cofmsgflowmodtest::tearDown() {}

void cofmsgflowmodtest::testFlowMod10() {
  testFlowMod(rofl::openflow10::OFP_VERSION, rofl::openflow10::OFPT_FLOW_MOD,
              0xa1a2a3a4);
}

void cofmsgflowmodtest::testFlowMod12() {
  testFlowMod(rofl::openflow12::OFP_VERSION, rofl::openflow12::OFPT_FLOW_MOD,
              0xa1a2a3a4);
}

void cofmsgflowmodtest::testFlowMod13() {
  testFlowMod(rofl::openflow13::OFP_VERSION, rofl::openflow13::OFPT_FLOW_MOD,
              0xa1a2a3a4);
}

void cofmsgflowmodtest::testFlowMod(uint8_t version, uint8_t type,
                                    uint32_t xid) {
...
void cofmsgflowmodtest::testFlowModParser10() {
  ...
  flowmod.set_instructions().add_inst_goto_table().set_table_id(0xee);
  size_t msglen =
      sizeof(struct rofl::openflow10::ofp_flow_mod) + flowmod.length();
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow10::ofp_flow_mod);
       i++) {
  ...
  for (unsigned int i = sizeof(struct rofl::openflow10::ofp_flow_mod);
       i == msglen; i++) {
    rofl::openflow::cofmsg_flow_mod msg;
    try {
      hdr->length = htobe16(i);
...
void cofmsgflowmodtest::testFlowModParser12() {
  ...
  size_t msglen =
      sizeof(struct rofl::openflow12::ofp_flow_mod) + flowmod.length();
  size_t memlen = 2 * msglen /*test overhead*/;
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow12::ofp_flow_mod);
       i++) {
    rofl::openflow::cofmsg_flow_mod msg;
    try {
      hdr->length = htobe16(i);
  ...
  for (unsigned int i = sizeof(struct rofl::openflow12::ofp_flow_mod);
       i == msglen; i++) {
    rofl::openflow::cofmsg_flow_mod msg;
    try {
      hdr->length = htobe16(i);
...
void cofmsgflowmodtest::testFlowModParser13() {
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_flow_mod);
       i++) {
    rofl::openflow::cofmsg_flow_mod msg;
    try {
      hdr->length = htobe16(i);
  ...
  for (unsigned int i = sizeof(struct rofl::openflow13::ofp_flow_mod);
       i == msglen; i++) {
    rofl::openflow::cofmsg_flow_mod msg;
    try {
      hdr->length = htobe16(i);
