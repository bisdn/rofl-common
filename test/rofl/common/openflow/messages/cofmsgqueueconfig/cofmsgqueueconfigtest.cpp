...
void
cofmsgqueueconfigtest::setUp()
{}



void
cofmsgqueueconfigtest::tearDown()
{}



void
cofmsgqueueconfigtest::testRequest10()
{
	testRequest(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_QUEUE_GET_CONFIG_REQUEST,
			0xa1a2a3a4);
}
...
void
cofmsgqueueconfigtest::testRequest12()
{
	testRequest(
...
void
cofmsgqueueconfigtest::testRequest13()
{
	testRequest(
...
void
cofmsgqueueconfigtest::testRequest(
...
void
cofmsgqueueconfigtest::testReply10()
{
	testReply(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_QUEUE_GET_CONFIG_REPLY,
			0xa1a2a3a4);
}
...
void
cofmsgqueueconfigtest::testReply12()
{
	testReply(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_QUEUE_GET_CONFIG_REPLY,
			0xa1a2a3a4);
}
...
void
cofmsgqueueconfigtest::testReply13()
{
	testReply(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_QUEUE_GET_CONFIG_REPLY,
			0xa1a2a3a4);
}
...
void
cofmsgqueueconfigtest::testReply(
		uint8_t version, uint8_t type, uint32_t xid)
{
	uint32_t portno = 0x61626364;
	rofl::openflow::cofpacket_queues queues(version);
	rofl::openflow::cofmsg_queue_get_config_reply msg1(version, xid, portno, queues);
...
void
cofmsgqueueconfigtest::testRequestParser10()
{
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow10::ofp_queue_get_config_request); i++) {
		...
		try {
			...
			CPPUNIT_ASSERT(i < sizeof(struct rofl::openflow10::ofp_queue_get_config_request));
		}
	}

	for (unsigned int i = sizeof(struct rofl::openflow10::ofp_queue_get_config_request); i == msglen; i++) {
		rofl::openflow::cofmsg_queue_get_config_request msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgqueueconfigtest::testRequestParser12()
{
	testRequestParser(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_QUEUE_GET_CONFIG_REQUEST,
...
void
cofmsgqueueconfigtest::testRequestParser13()
{
	testRequestParser(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_QUEUE_GET_CONFIG_REQUEST,
...
void
cofmsgqueueconfigtest::testRequestParser(
		uint8_t version, uint8_t type, uint32_t xid)
{
	uint32_t portno = 0x61626364;
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_queue_get_config_request); i++) {
		rofl::openflow::cofmsg_queue_get_config_request msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
			...
			CPPUNIT_ASSERT(i < sizeof(struct rofl::openflow13::ofp_queue_get_config_request));
		}
	}

	for (unsigned int i = sizeof(struct rofl::openflow13::ofp_queue_get_config_request); i == msglen; i++) {
		rofl::openflow::cofmsg_queue_get_config_request msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgqueueconfigtest::testReplyParser10()
{
	uint8_t version = rofl::openflow10::OFP_VERSION;
	...
	size_t msglen = sizeof(struct rofl::openflow10::ofp_queue_get_config_reply) + queues.length();
	size_t memlen = 2*msglen/*test overhead*/;
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow10::ofp_queue_get_config_reply); i++) {
		...
		try {
			...
			CPPUNIT_ASSERT(i < sizeof(struct rofl::openflow10::ofp_queue_get_config_reply));
		}
	}

	for (unsigned int i = sizeof(struct rofl::openflow10::ofp_queue_get_config_reply); i == msglen; i++) {
		rofl::openflow::cofmsg_queue_get_config_reply msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgqueueconfigtest::testReplyParser12()
{
	testReplyParser(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_QUEUE_GET_CONFIG_REPLY,
...
void
cofmsgqueueconfigtest::testReplyParser13()
{
	testReplyParser(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_QUEUE_GET_CONFIG_REPLY,
...
void
cofmsgqueueconfigtest::testReplyParser(
		uint8_t version, uint8_t type, uint32_t xid)
{
	uint32_t portno = 0x61626364;
	rofl::openflow::cofpacket_queues queues(version);
	queues.add_pqueue(0, 0x71727374);
	size_t msglen = sizeof(struct rofl::openflow13::ofp_queue_get_config_reply) + queues.length();
	size_t memlen = 2*msglen/*test overhead*/;
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_queue_get_config_reply); i++) {
		rofl::openflow::cofmsg_queue_get_config_reply msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
			...
			CPPUNIT_ASSERT(i < sizeof(struct rofl::openflow13::ofp_queue_get_config_reply));
		}
	}

	for (unsigned int i = sizeof(struct rofl::openflow13::ofp_queue_get_config_reply); i == msglen; i++) {
		rofl::openflow::cofmsg_queue_get_config_reply msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
