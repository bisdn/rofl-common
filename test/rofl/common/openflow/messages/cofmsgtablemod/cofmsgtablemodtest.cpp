...
void cofmsgtablemodtest::setUp() {}

void cofmsgtablemodtest::tearDown() {}

void cofmsgtablemodtest::testTableMod12() {
  testTableMod(rofl::openflow12::OFP_VERSION, rofl::openflow12::OFPT_TABLE_MOD,
               0xa1a2a3a4);
}

void cofmsgtablemodtest::testTableMod13() {
  testTableMod(rofl::openflow13::OFP_VERSION, rofl::openflow13::OFPT_TABLE_MOD,
               0xa1a2a3a4);
}

void cofmsgtablemodtest::testTableMod(uint8_t version, uint8_t type,
                                      uint32_t xid) {
...
void cofmsgtablemodtest::testTableModParser12() {
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow12::ofp_table_mod);
       i++) {
  ...
  for (unsigned int i = sizeof(struct rofl::openflow12::ofp_table_mod);
       i == msglen; i++) {
    rofl::openflow::cofmsg_table_mod msg;
    try {
      hdr->header.length = htobe16(i);
...
void cofmsgtablemodtest::testTableModParser13() {
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_table_mod);
       i++) {
    rofl::openflow::cofmsg_table_mod msg;
    try {
      hdr->header.length = htobe16(i);
  ...
  for (unsigned int i = sizeof(struct rofl::openflow13::ofp_table_mod);
       i == msglen; i++) {
    rofl::openflow::cofmsg_table_mod msg;
    try {
      hdr->header.length = htobe16(i);
