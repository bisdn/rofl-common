/*
 * cofmsgexperimenterstatstest.cpp
 *
...
#include "cofmsgexperimenterstatstest.hpp"

using namespace rofl::openflow;

CPPUNIT_TEST_SUITE_REGISTRATION(cofmsgexperimenterstatstest);

void cofmsgexperimenterstatstest::setUp() {}

void cofmsgexperimenterstatstest::tearDown() {}

void cofmsgexperimenterstatstest::testRequest10() {
  testRequest(rofl::openflow10::OFP_VERSION,
              rofl::openflow10::OFPT_STATS_REQUEST, 0xa1a2a3a4,
              rofl::openflow10::OFPST_VENDOR, 0xb1b2);
}

void cofmsgexperimenterstatstest::testRequest12() {
  testRequest(rofl::openflow12::OFP_VERSION,
              rofl::openflow12::OFPT_STATS_REQUEST, 0xa1a2a3a4,
              rofl::openflow12::OFPST_EXPERIMENTER, 0xb1b2);
}

void cofmsgexperimenterstatstest::testRequest13() {
  testRequest(rofl::openflow13::OFP_VERSION,
              rofl::openflow13::OFPT_MULTIPART_REQUEST, 0xa1a2a3a4,
              rofl::openflow13::OFPMP_EXPERIMENTER, 0xb1b2);
}

void cofmsgexperimenterstatstest::testRequest(uint8_t version, uint8_t type,
  ...
  rofl::openflow::cofmsg_experimenter_stats_request msg1(
      version, xid, stats_flags, exp_id, exp_type, exp_body);
  rofl::openflow::cofmsg_experimenter_stats_request msg2;
...
void cofmsgexperimenterstatstest::testReply10() {
  testReply(rofl::openflow10::OFP_VERSION, rofl::openflow10::OFPT_STATS_REPLY,
            0xa1a2a3a4, rofl::openflow10::OFPST_VENDOR, 0xb1b2);
}

void cofmsgexperimenterstatstest::testReply12() {
  testReply(rofl::openflow12::OFP_VERSION, rofl::openflow12::OFPT_STATS_REPLY,
            0xa1a2a3a4, rofl::openflow12::OFPST_EXPERIMENTER, 0xb1b2);
}

void cofmsgexperimenterstatstest::testReply13() {
  testReply(rofl::openflow13::OFP_VERSION,
            rofl::openflow13::OFPT_MULTIPART_REPLY, 0xa1a2a3a4,
            rofl::openflow13::OFPMP_EXPERIMENTER, 0xb1b2);
}

void cofmsgexperimenterstatstest::testReply(uint8_t version, uint8_t type,
                                            uint32_t xid, uint16_t stats_type,
                                            uint16_t stats_flags) {
  uint32_t exp_id = 0xe0e1e2e3;
  uint32_t exp_type = 0xf0f1f2f3;
  ...
  rofl::openflow::cofmsg_experimenter_stats_reply msg1(
      version, xid, stats_flags, exp_id, exp_type, exp_body);
  rofl::openflow::cofmsg_experimenter_stats_reply msg2;
  rofl::cmemory mem(msg1.length());
...
void cofmsgexperimenterstatstest::testRequestParser10() {
  ...
  size_t memlen = msglen + exp_body.length() + /*test overhead*/ 4;

  rofl::cmemory mem(memlen);
  struct rofl::openflow10::ofp_stats_request *stats =
      (struct rofl::openflow10::ofp_stats_request *)(mem.somem());
  struct rofl::openflow10::ofp_vendor_stats_header *vendor =
      (struct rofl::openflow10::ofp_vendor_stats_header *)(stats->body);
  uint8_t *body = (uint8_t *)(vendor + 1);
  ...
  stats->type = htobe16(rofl::openflow10::OFPST_VENDOR);
  stats->flags = htobe16(0xb1b2);
  vendor->vendor = htobe32(exp_id);
...
void cofmsgexperimenterstatstest::testRequestParser12() {
  ...
  size_t msglen =
      ...
      sizeof(struct rofl::openflow12::ofp_experimenter_stats_header);
  size_t memlen = msglen + exp_body.length() + /*test overhead*/ 4;

  rofl::cmemory mem(memlen);
  struct rofl::openflow12::ofp_stats_request *stats =
      (struct rofl::openflow12::ofp_stats_request *)(mem.somem());
  struct rofl::openflow12::ofp_experimenter_stats_header *vendor =
      (struct rofl::openflow12::ofp_experimenter_stats_header *)(stats->body);
  uint8_t *body = (uint8_t *)(vendor + 1);
  ...
  stats->header.xid = htobe32(0xa0a1a2a3);
  stats->header.length = htobe16(0);
  stats->type = htobe16(rofl::openflow12::OFPST_EXPERIMENTER);
  stats->flags = htobe16(0xb1b2);
  vendor->experimenter = htobe32(exp_id);
  vendor->exp_type = htobe32(exp_type);
  exp_body.pack(body, exp_body.length());
...
void cofmsgexperimenterstatstest::testRequestParser13() {
  ...
  struct rofl::openflow13::ofp_experimenter_multipart_header *vendor =
  ...
  stats->header.xid = htobe32(0xa0a1a2a3);
  stats->header.length = htobe16(0);
  stats->type = htobe16(rofl::openflow13::OFPMP_EXPERIMENTER);
  stats->flags = htobe16(0xb1b2);
  vendor->experimenter = htobe32(exp_id);
  vendor->exp_type = htobe32(exp_type);
  exp_body.pack(body, exp_body.length());
...
void cofmsgexperimenterstatstest::testReplyParser10() {
  uint8_t version = rofl::openflow10::OFP_VERSION;
  uint32_t exp_id = 0xe0e1e2e3;
  rofl::cmemory exp_body(64);
  for (unsigned int i = 0; i < exp_body.length(); ++i) {
...
void cofmsgexperimenterstatstest::testReplyParser12() {
  uint8_t version = rofl::openflow12::OFP_VERSION;
  uint32_t exp_id = 0xe0e1e2e3;
  uint32_t exp_type = 0xf0f1f2f3;
  rofl::cmemory exp_body(64);
  ...
  size_t msglen =
      sizeof(struct rofl::openflow12::ofp_stats_reply) +
      sizeof(struct rofl::openflow12::ofp_experimenter_stats_header);
  size_t memlen = msglen + exp_body.length() + /*test overhead*/ 4;
...
void cofmsgexperimenterstatstest::testReplyParser13() {
  uint8_t version = rofl::openflow13::OFP_VERSION;
  uint32_t exp_id = 0xe0e1e2e3;
  uint32_t exp_type = 0xf0f1f2f3;
  rofl::cmemory exp_body(64);
