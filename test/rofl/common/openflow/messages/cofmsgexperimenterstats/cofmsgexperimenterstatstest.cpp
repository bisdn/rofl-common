/*
 * cofmsgexperimenterstatstest.cpp
 *
 *  Created on: Apr 26, 2015
 *      Author: andi
 */

#include <stdlib.h>

#include <cppunit/extensions/TestFactoryRegistry.h>
#include <cppunit/ui/text/TestRunner.h>

#include "cofmsgexperimenterstatstest.hpp"

using namespace rofl::openflow;

CPPUNIT_TEST_SUITE_REGISTRATION( cofmsgexperimenterstatstest );

void
cofmsgexperimenterstatstest::setUp()
{}



void
cofmsgexperimenterstatstest::tearDown()
{}



void
cofmsgexperimenterstatstest::testRequest10()
{
	testRequest(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_STATS_REQUEST,
			0xa1a2a3a4,
			rofl::openflow10::OFPST_VENDOR,
			0xb1b2);
}



void
cofmsgexperimenterstatstest::testRequest12()
{
	testRequest(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_STATS_REQUEST,
			0xa1a2a3a4,
			rofl::openflow12::OFPST_EXPERIMENTER,
			0xb1b2);
}



void
cofmsgexperimenterstatstest::testRequest13()
{
	testRequest(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_MULTIPART_REQUEST,
			0xa1a2a3a4,
			rofl::openflow13::OFPMP_EXPERIMENTER,
			0xb1b2);
}



void
cofmsgexperimenterstatstest::testRequest(
		uint8_t version, uint8_t type, uint32_t xid, uint16_t stats_type, uint16_t stats_flags)
{
	uint32_t exp_id = 0xe0e1e2e3;
	uint32_t exp_type = 0xf0f1f2f3;
	rofl::cmemory exp_body(64);
	for (int i = 0; i < exp_body.length(); ++i) {
		exp_body[i] = i % 16;
	}
	rofl::openflow::cofmsg_experimenter_stats_request msg1(version, xid, stats_flags, exp_id, exp_type, exp_body);
	rofl::openflow::cofmsg_experimenter_stats_request msg2;
	rofl::cmemory mem(msg1.length());

	try {
		msg1.pack(mem.somem(), mem.length());
		msg2.unpack(mem.somem(), mem.length());

		CPPUNIT_ASSERT(msg2.get_version() == version);
		CPPUNIT_ASSERT(msg2.get_type() == type);
		CPPUNIT_ASSERT(msg2.get_length() == msg1.length());
		CPPUNIT_ASSERT(msg2.get_xid() == xid);
		CPPUNIT_ASSERT(msg2.get_stats_type() == stats_type);
		CPPUNIT_ASSERT(msg2.get_stats_flags() == stats_flags);
		CPPUNIT_ASSERT(msg2.get_exp_id() == exp_id);
		CPPUNIT_ASSERT(msg2.get_exp_body() == exp_body);

	} catch (...) {
		std::cerr << ">>> request <<<" << std::endl << msg1;
		std::cerr << ">>> memory <<<" << std::endl << mem;
		std::cerr << ">>> clone <<<" << std::endl << msg2;
		throw;
	}

}



void
cofmsgexperimenterstatstest::testReply10()
{
	testReply(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_STATS_REPLY,
			0xa1a2a3a4,
			rofl::openflow10::OFPST_VENDOR,
			0xb1b2);
}



void
cofmsgexperimenterstatstest::testReply12()
{
	testReply(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_STATS_REPLY,
			0xa1a2a3a4,
			rofl::openflow12::OFPST_EXPERIMENTER,
			0xb1b2);
}



void
cofmsgexperimenterstatstest::testReply13()
{
	testReply(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_MULTIPART_REPLY,
			0xa1a2a3a4,
			rofl::openflow13::OFPMP_EXPERIMENTER,
			0xb1b2);
}



void
cofmsgexperimenterstatstest::testReply(
		uint8_t version, uint8_t type, uint32_t xid, uint16_t stats_type, uint16_t stats_flags)
{
	uint32_t exp_id = 0xe0e1e2e3;
	uint32_t exp_type = 0xf0f1f2f3;
	rofl::cmemory exp_body(64);
	for (int i = 0; i < exp_body.length(); ++i) {
		exp_body[i] = i % 16;
	}
	rofl::openflow::cofmsg_experimenter_stats_reply msg1(version, xid, stats_flags, exp_id, exp_type, exp_body);
	rofl::openflow::cofmsg_experimenter_stats_reply msg2;
	rofl::cmemory mem(msg1.length());

	try {
		msg1.pack(mem.somem(), mem.length());
		msg2.unpack(mem.somem(), mem.length());

		CPPUNIT_ASSERT(msg2.get_version() == version);
		CPPUNIT_ASSERT(msg2.get_type() == type);
		CPPUNIT_ASSERT(msg2.get_length() == msg1.length());
		CPPUNIT_ASSERT(msg2.get_xid() == xid);
		CPPUNIT_ASSERT(msg2.get_stats_type() == stats_type);
		CPPUNIT_ASSERT(msg2.get_stats_flags() == stats_flags);
		CPPUNIT_ASSERT(msg2.get_exp_id() == exp_id);
		CPPUNIT_ASSERT(msg2.get_exp_body() == exp_body);

	} catch (...) {
		std::cerr << ">>> request <<<" << std::endl << msg1;
		std::cerr << ">>> memory <<<" << std::endl << mem;
		std::cerr << ">>> clone <<<" << std::endl << msg2;
		throw;
	}
}



void
cofmsgexperimenterstatstest::testRequestParser10()
{
	uint8_t version = rofl::openflow10::OFP_VERSION;
	uint32_t exp_id = 0xe0e1e2e3;
	rofl::cmemory exp_body(64);
	for (int i = 0; i < exp_body.length(); ++i) {
		exp_body[i] = i % 16;
	}

	size_t msglen = sizeof(struct rofl::openflow10::ofp_stats_request) +
					sizeof(struct rofl::openflow10::ofp_vendor_stats_header);
	size_t memlen = msglen + exp_body.length() + /*test overhead*/4;

	rofl::cmemory mem(memlen);
	struct rofl::openflow10::ofp_stats_request* stats =
			(struct rofl::openflow10::ofp_stats_request*)(mem.somem());
	struct rofl::openflow10::ofp_vendor_stats_header* vendor =
			(struct rofl::openflow10::ofp_vendor_stats_header*)(stats->body);
	uint8_t* body = (uint8_t*)(vendor + 1);

	stats->header.version = version;
	stats->header.type = rofl::openflow10::OFPT_STATS_REQUEST;
	stats->header.xid = htobe32(0xa0a1a2a3);
	stats->header.length = htobe16(0);
	stats->type = htobe16(rofl::openflow10::OFPST_VENDOR);
	stats->flags = htobe16(0xb1b2);
	vendor->vendor = htobe32(exp_id);
	exp_body.pack(body, exp_body.length());


	for (int i = 1; i < msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_request msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> request <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);

		} catch (rofl::eBadSyntaxTooShort& e) {
			CPPUNIT_ASSERT(i < msglen);
		}
	}

	for (int i = msglen; i == msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_request msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() exception seen" << std::endl;
			std::cerr << ">>> request <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);
		}
	}

	for (int i = msglen + 1; i < memlen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_request msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {
			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> request <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			CPPUNIT_ASSERT(false);
		}
	}
}



void
cofmsgexperimenterstatstest::testRequestParser12()
{
	uint8_t version = rofl::openflow12::OFP_VERSION;
	uint32_t exp_id = 0xe0e1e2e3;
	uint32_t exp_type = 0xf0f1f2f3;
	rofl::cmemory exp_body(64);
	for (int i = 0; i < exp_body.length(); ++i) {
		exp_body[i] = i % 16;
	}

	size_t msglen = sizeof(struct rofl::openflow12::ofp_stats_request) +
					sizeof(struct rofl::openflow12::ofp_experimenter_stats_header);
	size_t memlen = msglen + exp_body.length() + /*test overhead*/4;

	rofl::cmemory mem(memlen);
	struct rofl::openflow12::ofp_stats_request* stats =
			(struct rofl::openflow12::ofp_stats_request*)(mem.somem());
	struct rofl::openflow12::ofp_experimenter_stats_header* vendor =
			(struct rofl::openflow12::ofp_experimenter_stats_header*)(stats->body);
	uint8_t* body = (uint8_t*)(vendor + 1);

	stats->header.version = version;
	stats->header.type = rofl::openflow12::OFPT_STATS_REQUEST;
	stats->header.xid = htobe32(0xa0a1a2a3);
	stats->header.length = htobe16(0);
	stats->type = htobe16(rofl::openflow12::OFPST_EXPERIMENTER);
	stats->flags = htobe16(0xb1b2);
	vendor->experimenter = htobe32(exp_id);
	vendor->exp_type = htobe32(exp_type);
	exp_body.pack(body, exp_body.length());


	for (int i = 1; i < msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_request msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> request <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);

		} catch (rofl::eBadSyntaxTooShort& e) {
			CPPUNIT_ASSERT(i < msglen);
		}
	}

	for (int i = msglen; i == msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_request msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() exception seen" << std::endl;
			std::cerr << ">>> request <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);
		}
	}

	for (int i = msglen + 1; i < memlen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_request msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {
			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> request <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			CPPUNIT_ASSERT(false);
		}
	}
}



void
cofmsgexperimenterstatstest::testRequestParser13()
{
	uint8_t version = rofl::openflow13::OFP_VERSION;
	uint32_t exp_id = 0xe0e1e2e3;
	uint32_t exp_type = 0xf0f1f2f3;
	rofl::cmemory exp_body(64);
	for (int i = 0; i < exp_body.length(); ++i) {
		exp_body[i] = i % 16;
	}

	size_t msglen = sizeof(struct rofl::openflow13::ofp_multipart_request) +
					sizeof(struct rofl::openflow13::ofp_experimenter_multipart_header);
	size_t memlen = msglen + exp_body.length() + /*test overhead*/4;

	rofl::cmemory mem(memlen);
	struct rofl::openflow13::ofp_multipart_request* stats =
			(struct rofl::openflow13::ofp_multipart_request*)(mem.somem());
	struct rofl::openflow13::ofp_experimenter_multipart_header* vendor =
			(struct rofl::openflow13::ofp_experimenter_multipart_header*)(stats->body);
	uint8_t* body = (uint8_t*)(vendor + 1);

	stats->header.version = version;
	stats->header.type = rofl::openflow13::OFPT_MULTIPART_REQUEST;
	stats->header.xid = htobe32(0xa0a1a2a3);
	stats->header.length = htobe16(0);
	stats->type = htobe16(rofl::openflow13::OFPMP_EXPERIMENTER);
	stats->flags = htobe16(0xb1b2);
	vendor->experimenter = htobe32(exp_id);
	vendor->exp_type = htobe32(exp_type);
	exp_body.pack(body, exp_body.length());


	for (int i = 1; i < msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_request msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> request <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);

		} catch (rofl::eBadSyntaxTooShort& e) {
			CPPUNIT_ASSERT(i < msglen);
		}
	}

	for (int i = msglen; i == msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_request msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() exception seen" << std::endl;
			std::cerr << ">>> request <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);
		}
	}

	for (int i = msglen + 1; i < memlen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_request msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {
			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> request <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			CPPUNIT_ASSERT(false);
		}
	}}



void
cofmsgexperimenterstatstest::testReplyParser10()
{
	uint8_t version = rofl::openflow10::OFP_VERSION;
	uint32_t exp_id = 0xe0e1e2e3;
	rofl::cmemory exp_body(64);
	for (int i = 0; i < exp_body.length(); ++i) {
		exp_body[i] = i % 16;
	}

	size_t msglen = sizeof(struct rofl::openflow10::ofp_stats_reply) +
					sizeof(struct rofl::openflow10::ofp_vendor_stats_header);
	size_t memlen = msglen + exp_body.length() + /*test overhead*/4;

	rofl::cmemory mem(memlen);
	struct rofl::openflow10::ofp_stats_reply* stats =
			(struct rofl::openflow10::ofp_stats_reply*)(mem.somem());
	struct rofl::openflow10::ofp_vendor_stats_header* vendor =
			(struct rofl::openflow10::ofp_vendor_stats_header*)(stats->body);
	uint8_t* body = (uint8_t*)(vendor + 1);

	stats->header.version = version;
	stats->header.type = rofl::openflow10::OFPT_STATS_REPLY;
	stats->header.xid = htobe32(0xa0a1a2a3);
	stats->header.length = htobe16(0);
	stats->type = htobe16(rofl::openflow10::OFPST_VENDOR);
	stats->flags = htobe16(0xb1b2);
	vendor->vendor = htobe32(exp_id);
	exp_body.pack(body, exp_body.length());


	for (int i = 1; i < msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_reply msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> reply <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);

		} catch (rofl::eBadSyntaxTooShort& e) {
			CPPUNIT_ASSERT(i < msglen);
		}
	}

	for (int i = msglen; i == msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_reply msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() exception seen" << std::endl;
			std::cerr << ">>> reply <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);
		}
	}

	for (int i = msglen + 1; i < memlen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_reply msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {
			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> reply <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			CPPUNIT_ASSERT(false);
		}
	}
}



void
cofmsgexperimenterstatstest::testReplyParser12()
{
	uint8_t version = rofl::openflow12::OFP_VERSION;
	uint32_t exp_id = 0xe0e1e2e3;
	uint32_t exp_type = 0xf0f1f2f3;
	rofl::cmemory exp_body(64);
	for (int i = 0; i < exp_body.length(); ++i) {
		exp_body[i] = i % 16;
	}

	size_t msglen = sizeof(struct rofl::openflow12::ofp_stats_reply) +
					sizeof(struct rofl::openflow12::ofp_experimenter_stats_header);
	size_t memlen = msglen + exp_body.length() + /*test overhead*/4;

	rofl::cmemory mem(memlen);
	struct rofl::openflow12::ofp_stats_reply* stats =
			(struct rofl::openflow12::ofp_stats_reply*)(mem.somem());
	struct rofl::openflow12::ofp_experimenter_stats_header* vendor =
			(struct rofl::openflow12::ofp_experimenter_stats_header*)(stats->body);
	uint8_t* body = (uint8_t*)(vendor + 1);

	stats->header.version = version;
	stats->header.type = rofl::openflow12::OFPT_STATS_REPLY;
	stats->header.xid = htobe32(0xa0a1a2a3);
	stats->header.length = htobe16(0);
	stats->type = htobe16(rofl::openflow12::OFPST_EXPERIMENTER);
	stats->flags = htobe16(0xb1b2);
	vendor->experimenter = htobe32(exp_id);
	vendor->exp_type = htobe32(exp_type);
	exp_body.pack(body, exp_body.length());


	for (int i = 1; i < msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_reply msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> reply <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);

		} catch (rofl::eBadSyntaxTooShort& e) {
			CPPUNIT_ASSERT(i < msglen);
		}
	}

	for (int i = msglen; i == msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_reply msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() exception seen" << std::endl;
			std::cerr << ">>> reply <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);
		}
	}

	for (int i = msglen + 1; i < memlen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_reply msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {
			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> reply <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			CPPUNIT_ASSERT(false);
		}
	}
}



void
cofmsgexperimenterstatstest::testReplyParser13()
{
	uint8_t version = rofl::openflow13::OFP_VERSION;
	uint32_t exp_id = 0xe0e1e2e3;
	uint32_t exp_type = 0xf0f1f2f3;
	rofl::cmemory exp_body(64);
	for (int i = 0; i < exp_body.length(); ++i) {
		exp_body[i] = i % 16;
	}

	size_t msglen = sizeof(struct rofl::openflow13::ofp_multipart_reply) +
					sizeof(struct rofl::openflow13::ofp_experimenter_multipart_header);
	size_t memlen = msglen + exp_body.length() + /*test overhead*/4;

	rofl::cmemory mem(memlen);
	struct rofl::openflow13::ofp_multipart_reply* stats =
			(struct rofl::openflow13::ofp_multipart_reply*)(mem.somem());
	struct rofl::openflow13::ofp_experimenter_multipart_header* vendor =
			(struct rofl::openflow13::ofp_experimenter_multipart_header*)(stats->body);
	uint8_t* body = (uint8_t*)(vendor + 1);

	stats->header.version = version;
	stats->header.type = rofl::openflow13::OFPT_MULTIPART_REPLY;
	stats->header.xid = htobe32(0xa0a1a2a3);
	stats->header.length = htobe16(0);
	stats->type = htobe16(rofl::openflow13::OFPMP_EXPERIMENTER);
	stats->flags = htobe16(0xb1b2);
	vendor->experimenter = htobe32(exp_id);
	vendor->exp_type = htobe32(exp_type);
	exp_body.pack(body, exp_body.length());


	for (int i = 1; i < msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_reply msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> reply <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);

		} catch (rofl::eBadSyntaxTooShort& e) {
			CPPUNIT_ASSERT(i < msglen);
		}
	}

	for (int i = msglen; i == msglen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_reply msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {

			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() exception seen" << std::endl;
			std::cerr << ">>> reply <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			/* unpack() Must yield an axception */
			CPPUNIT_ASSERT(false);
		}
	}

	for (int i = msglen + 1; i < memlen; i++) {
		rofl::openflow::cofmsg_experimenter_stats_reply msg;
		try {
			stats->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);

		} catch (rofl::eBadSyntaxTooShort& e) {
			std::cerr << ">>> testing length values (len: " << i << ") <<< " << std::endl;
			std::cerr << "[FAILURE] unpack() no exception seen" << std::endl;
			std::cerr << ">>> reply <<<" << std::endl << msg;
			std::cerr << ">>> memory <<<" << std::endl << mem;

			CPPUNIT_ASSERT(false);
		}
	}
}



