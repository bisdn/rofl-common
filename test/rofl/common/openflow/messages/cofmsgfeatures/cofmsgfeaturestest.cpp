...
void
cofmsgfeaturestest::setUp()
{}



void
cofmsgfeaturestest::tearDown()
{}



void
cofmsgfeaturestest::testRequest10()
{
	testRequest(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_FEATURES_REQUEST,
			0xa1a2a3a4);
}
...
void
cofmsgfeaturestest::testRequest12()
{
	testRequest(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_FEATURES_REQUEST,
			0xa1a2a3a4);
}
...
void
cofmsgfeaturestest::testRequest13()
{
	testRequest(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_FEATURES_REQUEST,
			0xa1a2a3a4);
}
...
void
cofmsgfeaturestest::testRequest(
...
void
cofmsgfeaturestest::testReply10()
{
	testReply(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_FEATURES_REPLY,
			0xa1a2a3a4);
}
...
void
cofmsgfeaturestest::testReply12()
{
	testReply(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_FEATURES_REPLY,
			0xa1a2a3a4);
}
...
void
cofmsgfeaturestest::testReply13()
{
	testReply(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_FEATURES_REPLY,
			0xa1a2a3a4);
}
		...
		uint8_t version, uint8_t type, uint32_t xid)
{
	...
	rofl::openflow::cofmsg_features_reply msg1(version, xid, dpid, n_buffers, n_tables, auxid, capabilities, actions_bitmap, ports);
...
void
cofmsgfeaturestest::testRequestParser10()
{
	testRequestParser(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_FEATURES_REQUEST,
...
void
cofmsgfeaturestest::testRequestParser12()
{
	testRequestParser(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_FEATURES_REQUEST,
...
void
cofmsgfeaturestest::testRequestParser13()
{
	testRequestParser(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_FEATURES_REQUEST,
...
void
cofmsgfeaturestest::testRequestParser(
		uint8_t version, uint8_t type, uint32_t xid)
{
...
void
cofmsgfeaturestest::testReplyParser10()
{
	...
	size_t msglen = sizeof(struct rofl::openflow10::ofp_switch_features) + ports.length();
	size_t memlen = 2*msglen/*test overhead*/;
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow10::ofp_switch_features); i++) {
	...
	for (unsigned int i = sizeof(struct rofl::openflow10::ofp_switch_features); i == msglen; i++) {
		rofl::openflow::cofmsg_features_reply msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgfeaturestest::testReplyParser12()
{
	...
	size_t msglen = sizeof(struct rofl::openflow12::ofp_switch_features) + ports.length();
	size_t memlen = 2*msglen/*test overhead*/;
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow12::ofp_switch_features); i++) {
		rofl::openflow::cofmsg_features_reply msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
	...
	for (unsigned int i = sizeof(struct rofl::openflow12::ofp_switch_features); i == msglen; i++) {
		rofl::openflow::cofmsg_features_reply msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgfeaturestest::testReplyParser13()
{
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_switch_features); i++) {
		rofl::openflow::cofmsg_features_reply msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
	...
	for (unsigned int i = sizeof(struct rofl::openflow13::ofp_switch_features); i == msglen; i++) {
		rofl::openflow::cofmsg_features_reply msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
