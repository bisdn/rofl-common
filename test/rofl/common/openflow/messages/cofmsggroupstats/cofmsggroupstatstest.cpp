...
void cofmsggroupstatstest::setUp() {}

void cofmsggroupstatstest::tearDown() {}

void cofmsggroupstatstest::testRequest12() {
  testRequest(rofl::openflow12::OFP_VERSION,
              rofl::openflow12::OFPT_STATS_REQUEST, 0xa1a2a3a4,
              rofl::openflow12::OFPST_GROUP, 0xb1b2);
}

void cofmsggroupstatstest::testRequest13() {
  testRequest(rofl::openflow13::OFP_VERSION,
              rofl::openflow13::OFPT_MULTIPART_REQUEST, 0xa1a2a3a4,
              rofl::openflow13::OFPMP_GROUP, 0xb1b2);
}

void cofmsggroupstatstest::testRequest(uint8_t version, uint8_t type,
...
void cofmsggroupstatstest::testReply12() {
  testReply(rofl::openflow12::OFP_VERSION, rofl::openflow12::OFPT_STATS_REPLY,
            0xa1a2a3a4, rofl::openflow12::OFPST_GROUP, 0xb1b2);
}

void cofmsggroupstatstest::testReply13() {
  testReply(rofl::openflow13::OFP_VERSION,
            rofl::openflow13::OFPT_MULTIPART_REPLY, 0xa1a2a3a4,
            rofl::openflow13::OFPMP_GROUP, 0xb1b2);
}

void cofmsggroupstatstest::testReply(uint8_t version, uint8_t type,
                                     uint32_t xid, uint16_t stats_type,
                                     uint16_t stats_flags) {
...
void cofmsggroupstatstest::testRequestParser12() {
  uint8_t version = rofl::openflow12::OFP_VERSION;

  cofgroup_stats_request group(version);

  size_t msglen =
      sizeof(struct rofl::openflow12::ofp_stats_request) + group.length();
...
void cofmsggroupstatstest::testRequestParser13() {
  uint8_t version = rofl::openflow13::OFP_VERSION;

  cofgroup_stats_request group(version);

  size_t msglen =
      sizeof(struct rofl::openflow13::ofp_multipart_request) + group.length();
  size_t memlen = msglen + /*test overhead*/ 4;
...
void cofmsggroupstatstest::testReplyParser12() {
  uint8_t version = rofl::openflow12::OFP_VERSION;
  ...
  for (unsigned int i = msglen + 1; i < memlen; i++) {
    ...
    try {
      ...
      std::cerr << ">>> testing length values (len: " << i << ") <<< "
                << std::endl;
      std::cerr << "[FAILURE] unpack() exception seen: " << e.what()
                << std::endl;
      std::cerr << ">>> reply <<<" << std::endl << msg;
      std::cerr << ">>> memory <<<" << std::endl << mem;
...
void cofmsggroupstatstest::testReplyParser13() {
  uint8_t version = rofl::openflow13::OFP_VERSION;
  uint64_t byte_count = 0xb1b2b3b4b5b6b7b8;
