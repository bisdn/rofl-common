...
void
cofmsgportstatustest::setUp()
{}



void
cofmsgportstatustest::tearDown()
{}



void
cofmsgportstatustest::testPortStatus10()
{
	testPortStatus(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_PORT_STATUS,
			0xa1a2a3a4);
}
...
void
cofmsgportstatustest::testPortStatus12()
{
	testPortStatus(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_PORT_STATUS,
			0xa1a2a3a4);
}
...
void
cofmsgportstatustest::testPortStatus13()
{
	testPortStatus(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_PORT_STATUS,
			0xa1a2a3a4);
}
...
void
cofmsgportstatustest::testPortStatus(
...
void
cofmsgportstatustest::testPortStatusParser10()
{
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow10::ofp_header); i++) {
	...
	for (unsigned int i = sizeof(struct rofl::openflow10::ofp_header); i == msglen; i++) {
		rofl::openflow::cofmsg_port_status msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgportstatustest::testPortStatusParser12()
{
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow12::ofp_header); i++) {
		rofl::openflow::cofmsg_port_status msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
	...
	for (unsigned int i = sizeof(struct rofl::openflow12::ofp_header); i == msglen; i++) {
		rofl::openflow::cofmsg_port_status msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgportstatustest::testPortStatusParser13()
{
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_header); i++) {
		rofl::openflow::cofmsg_port_status msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
	...
	for (unsigned int i = sizeof(struct rofl::openflow13::ofp_header); i == msglen; i++) {
		rofl::openflow::cofmsg_port_status msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
