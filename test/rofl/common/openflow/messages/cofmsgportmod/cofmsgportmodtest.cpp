...
void
cofmsgportmodtest::setUp()
{}



void
cofmsgportmodtest::tearDown()
{}



void
cofmsgportmodtest::testPortMod10()
{
	testPortMod(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_PORT_MOD,
			0xa1a2a3a4);
}
...
void
cofmsgportmodtest::testPortMod12()
{
	testPortMod(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_PORT_MOD,
			0xa1a2a3a4);
}
...
void
cofmsgportmodtest::testPortMod13()
{
	testPortMod(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_PORT_MOD,
			0xa1a2a3a4);
}
		...
		uint8_t version, uint8_t type, uint32_t xid)
{
	...
	rofl::openflow::cofmsg_port_mod msg1(version, xid, port_no, lladdr, config, mask, advertise);
...
void
cofmsgportmodtest::testPortModParser10()
{
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow10::ofp_port_mod); i++) {
	...
	for (unsigned int i = sizeof(struct rofl::openflow10::ofp_port_mod); i == msglen; i++) {
		rofl::openflow::cofmsg_port_mod msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgportmodtest::testPortModParser12()
{
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow12::ofp_port_mod); i++) {
		rofl::openflow::cofmsg_port_mod msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
	...
	for (unsigned int i = sizeof(struct rofl::openflow12::ofp_port_mod); i == msglen; i++) {
		rofl::openflow::cofmsg_port_mod msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgportmodtest::testPortModParser13()
{
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_port_mod); i++) {
		rofl::openflow::cofmsg_port_mod msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
	...
	for (unsigned int i = sizeof(struct rofl::openflow13::ofp_port_mod); i == msglen; i++) {
		rofl::openflow::cofmsg_port_mod msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
