...
void
cofmsgexperimentertest::setUp()
{}



void
cofmsgexperimentertest::tearDown()
{}



void
cofmsgexperimentertest::testExperimenter10()
{
	testExperimenter(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_VENDOR,
			0xa1a2a3a4);
}
...
void
cofmsgexperimentertest::testExperimenter12()
{
	testExperimenter(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_EXPERIMENTER,
			0xa1a2a3a4);
}
...
void
cofmsgexperimentertest::testExperimenter13()
{
	testExperimenter(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_EXPERIMENTER,
			0xa1a2a3a4);
}
...
void
cofmsgexperimentertest::testExperimenter(
		uint8_t version, uint8_t type, uint32_t xid)
{
	...
	rofl::openflow::cofmsg_experimenter msg1(version, xid, exp_id, exp_type, body.somem(), body.length());
...
void
cofmsgexperimentertest::testExperimenterParser10()
{
	uint8_t version = rofl::openflow10::OFP_VERSION;
	uint8_t type = rofl::openflow10::OFPT_VENDOR;
	uint32_t xid = 0xa1a2a3a4;
	rofl::cmemory body(64);
	size_t msglen = sizeof(struct rofl::openflow10::ofp_vendor_header) + body.length();
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow10::ofp_vendor_header); i++) {
	...
	for (unsigned int i = sizeof(struct rofl::openflow10::ofp_vendor_header); i == msglen; i++) {
		rofl::openflow::cofmsg_experimenter msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgexperimentertest::testExperimenterParser12()
{
	uint8_t version = rofl::openflow12::OFP_VERSION;
	uint8_t type = rofl::openflow12::OFPT_EXPERIMENTER;
	uint32_t xid = 0xa1a2a3a4;
	rofl::cmemory body(64);
	size_t msglen = sizeof(struct rofl::openflow12::ofp_experimenter_header) + body.length();
	size_t memlen = 2*msglen/*test overhead*/;
	uint32_t exp_id = 0xc1c2c3c4;
	uint32_t exp_type = 0xd1d2d3d4;
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow12::ofp_experimenter_header); i++) {
		rofl::openflow::cofmsg_experimenter msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
			...
			CPPUNIT_ASSERT(i < sizeof(struct rofl::openflow12::ofp_experimenter_header));
		}
	}

	for (unsigned int i = sizeof(struct rofl::openflow12::ofp_experimenter_header); i == msglen; i++) {
		rofl::openflow::cofmsg_experimenter msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgexperimentertest::testExperimenterParser13()
{
	uint8_t version = rofl::openflow13::OFP_VERSION;
	uint8_t type = rofl::openflow13::OFPT_EXPERIMENTER;
	uint32_t xid = 0xa1a2a3a4;
	rofl::cmemory body(64);
	size_t msglen = sizeof(struct rofl::openflow13::ofp_experimenter_header) + body.length();
	size_t memlen = 2*msglen/*test overhead*/;
	uint32_t exp_id = 0xc1c2c3c4;
	uint32_t exp_type = 0xd1d2d3d4;
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_experimenter_header); i++) {
		rofl::openflow::cofmsg_experimenter msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
			...
			CPPUNIT_ASSERT(i < sizeof(struct rofl::openflow13::ofp_experimenter_header));
		}
	}

	for (unsigned int i = sizeof(struct rofl::openflow13::ofp_experimenter_header); i == msglen; i++) {
		rofl::openflow::cofmsg_experimenter msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
