...
void
cofmsgmetermodtest::setUp()
{}



void
cofmsgmetermodtest::tearDown()
{}



void
cofmsgmetermodtest::testMeterMod13()
{
	testMeterMod(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_METER_MOD,
			0xa1a2a3a4);
}
...
void
cofmsgmetermodtest::testMeterMod(
		uint8_t version, uint8_t type, uint32_t xid)
{
	...
	rofl::openflow::cofmsg_meter_mod msg1(version, xid, command, flags, meter_id, meter_bands);
	...
	try {
		...
		CPPUNIT_ASSERT(msg2.get_meter_bands().get_num_of_mbs() == meter_bands.get_num_of_mbs());
		CPPUNIT_ASSERT(msg2.get_meter_bands().get_version() == meter_bands.get_version());
...
void
cofmsgmetermodtest::testMeterModParser13()
{
	...
	size_t msglen = sizeof(struct rofl::openflow13::ofp_meter_mod) + meter_bands.length();
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_meter_mod); i++) {
	...
	for (unsigned int i = sizeof(struct rofl::openflow13::ofp_meter_mod); i == msglen; i++) {
		rofl::openflow::cofmsg_meter_mod msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
