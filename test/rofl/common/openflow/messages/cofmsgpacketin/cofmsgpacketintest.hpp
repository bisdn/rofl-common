...
class cofmsgpacketintest : public CppUnit::TestFixture {
...
private:
  void testPacketIn(uint8_t version, uint8_t type, uint32_t xid);
  void testPacketInParser(uint8_t version, uint8_t type, uint32_t xid);
};
