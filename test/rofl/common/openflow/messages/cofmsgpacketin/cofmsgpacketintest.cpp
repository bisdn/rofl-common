...
void
cofmsgpacketintest::setUp()
{}



void
cofmsgpacketintest::tearDown()
{}



void
cofmsgpacketintest::testPacketIn10()
{
	testPacketIn(
			rofl::openflow10::OFP_VERSION,
			rofl::openflow10::OFPT_PACKET_IN,
			0xa1a2a3a4);
}
...
void
cofmsgpacketintest::testPacketIn12()
{
	testPacketIn(
			rofl::openflow12::OFP_VERSION,
			rofl::openflow12::OFPT_PACKET_IN,
			0xa1a2a3a4);
}
...
void
cofmsgpacketintest::testPacketIn13()
{
	testPacketIn(
			rofl::openflow13::OFP_VERSION,
			rofl::openflow13::OFPT_PACKET_IN,
			0xa1a2a3a4);
}
...
void
cofmsgpacketintest::testPacketIn(
		uint8_t version, uint8_t type, uint32_t xid)
{
	...
	rofl::openflow::cofmsg_packet_in msg1(version, xid, buffer_id, total_len, reason, table_id, cookie, in_port, match, packet.soframe(), packet.length());
...
void
cofmsgpacketintest::testPacketInParser10()
{
	...
	size_t msglen = sizeof(struct rofl::openflow10::ofp_packet_in) + 2 + packet.length();
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow10::ofp_packet_in); i++) {
	...
	for (unsigned int i = sizeof(struct rofl::openflow10::ofp_packet_in); i == msglen; i++) {
		rofl::openflow::cofmsg_packet_in msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgpacketintest::testPacketInParser12()
{
	...
	size_t msglen = sizeof(struct rofl::openflow12::ofp_packet_in) + 2 + packet.length();
	size_t memlen = 2 * msglen/*test overhead*/;
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow12::ofp_packet_in); i++) {
		rofl::openflow::cofmsg_packet_in msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
	...
	for (unsigned int i = sizeof(struct rofl::openflow12::ofp_packet_in); i == msglen; i++) {
		rofl::openflow::cofmsg_packet_in msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
...
void
cofmsgpacketintest::testPacketInParser13()
{
	...
	size_t msglen = sizeof(struct rofl::openflow13::ofp_packet_in) + 2 + packet.length();
	size_t memlen = 2 * msglen/*test overhead*/;
	...
	for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_packet_in); i++) {
		rofl::openflow::cofmsg_packet_in msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
	...
	for (unsigned int i = sizeof(struct rofl::openflow13::ofp_packet_in); i == msglen; i++) {
		rofl::openflow::cofmsg_packet_in msg;
		try {
			hdr->header.length = htobe16(i);
			msg.unpack(mem.somem(), i);
