...
void cofmsggroupmodtest::setUp() {}

void cofmsggroupmodtest::tearDown() {}

void cofmsggroupmodtest::testGroupMod12() {
  testGroupMod(rofl::openflow12::OFP_VERSION, rofl::openflow12::OFPT_GROUP_MOD,
               0xa1a2a3a4);
}

void cofmsggroupmodtest::testGroupMod13() {
  testGroupMod(rofl::openflow13::OFP_VERSION, rofl::openflow13::OFPT_GROUP_MOD,
               0xa1a2a3a4);
}

void cofmsggroupmodtest::testGroupMod(uint8_t version, uint8_t type,
                                      uint32_t xid) {
  ...
  try {
    ...
    for (unsigned int i = 0; i < 4; i++) {
      CPPUNIT_ASSERT(
          msg2.get_groupmod().get_buckets().get_bucket(i).get_watch_port() ==
          watch_port);
      CPPUNIT_ASSERT(
          msg2.get_groupmod().get_buckets().get_bucket(i).get_watch_group() ==
          watch_group);
    }
...
void cofmsggroupmodtest::testGroupModParser12() {
  ...
  size_t msglen =
      sizeof(struct rofl::openflow12::ofp_group_mod) + groupmod.length();
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow12::ofp_group_mod);
       i++) {
  ...
  for (unsigned int i = sizeof(struct rofl::openflow12::ofp_group_mod);
       i == msglen; i++) {
    rofl::openflow::cofmsg_group_mod msg;
    try {
      hdr->length = htobe16(i);
...
void cofmsggroupmodtest::testGroupModParser13() {
  ...
  size_t msglen =
      sizeof(struct rofl::openflow13::ofp_group_mod) + groupmod.length();
  size_t memlen = 2 * msglen /*test overhead*/;
  ...
  for (unsigned int i = 1; i < sizeof(struct rofl::openflow13::ofp_group_mod);
       i++) {
    rofl::openflow::cofmsg_group_mod msg;
    try {
      hdr->length = htobe16(i);
  ...
  for (unsigned int i = sizeof(struct rofl::openflow13::ofp_group_mod);
       i == msglen; i++) {
    rofl::openflow::cofmsg_group_mod msg;
    try {
      hdr->length = htobe16(i);
