...
cofmeter_bands::cofmeter_bands(uint8_t of_version) : of_version(of_version) {}

cofmeter_bands::~cofmeter_bands() { clear(); }

cofmeter_bands::cofmeter_bands(cofmeter_bands const &mbs) { *this = mbs; }

cofmeter_bands &cofmeter_bands::operator=(cofmeter_bands const &mbs) {
  if (this == &mbs)
    return *this;
  ...
  for (std::map<unsigned int, cofmeter_band_drop>::const_iterator it =
           mbs.mbs_drop.begin();
       it != mbs.mbs_drop.end(); ++it) {
    mbs_drop[it->first] = it->second;
  }

  for (std::map<unsigned int, cofmeter_band_dscp_remark>::const_iterator it =
           mbs.mbs_dscp_remark.begin();
       it != mbs.mbs_dscp_remark.end(); ++it) {
    mbs_dscp_remark[it->first] = it->second;
  }

  for (std::map<unsigned int, cofmeter_band_experimenter>::const_iterator it =
           mbs.mbs_experimenter.begin();
       it != mbs.mbs_experimenter.end(); ++it) {
...
void cofmeter_bands::clear() {
  mbs_drop.clear();
  mbs_dscp_remark.clear();
  mbs_experimenter.clear();
}

unsigned int cofmeter_bands::get_num_of_mbs() const {
  return (mbs_drop.size() + mbs_dscp_remark.size() + mbs_experimenter.size());
}

cofmeter_band_drop &cofmeter_bands::add_meter_band_drop(unsigned int index) {
...
void cofmeter_bands::drop_meter_band_drop(unsigned int index) {
  if (mbs_drop.find(index) == mbs_drop.end()) {
    return;
  }
  mbs_drop.erase(index);
...
cofmeter_band_drop &cofmeter_bands::set_meter_band_drop(unsigned int index) {
  if (mbs_drop.find(index) == mbs_drop.end()) {
...
cofmeter_bands::get_meter_band_drop(unsigned int index) const {
  if (mbs_drop.find(index) == mbs_drop.end()) {
...
bool cofmeter_bands::has_meter_band_drop(unsigned int index) const {
  return (not(mbs_drop.find(index) == mbs_drop.end()));
}

cofmeter_band_dscp_remark &
cofmeter_bands::add_meter_band_dscp_remark(unsigned int index) {
...
void cofmeter_bands::drop_meter_band_dscp_remark(unsigned int index) {
  if (mbs_dscp_remark.find(index) == mbs_dscp_remark.end()) {
    return;
  }
  mbs_dscp_remark.erase(index);
...
cofmeter_band_dscp_remark &
cofmeter_bands::set_meter_band_dscp_remark(unsigned int index) {
  if (mbs_dscp_remark.find(index) == mbs_dscp_remark.end()) {
...
cofmeter_bands::get_meter_band_dscp_remark(unsigned int index) const {
  if (mbs_dscp_remark.find(index) == mbs_dscp_remark.end()) {
    throw eRofMeterBandNotFound();
  }
...
bool cofmeter_bands::has_meter_band_dscp_remark(unsigned int index) const {
  return (not(mbs_dscp_remark.find(index) == mbs_dscp_remark.end()));
}

cofmeter_band_experimenter &
cofmeter_bands::add_meter_band_experimenter(unsigned int index) {
...
void cofmeter_bands::drop_meter_band_experimenter(unsigned int index) {
  if (mbs_experimenter.find(index) == mbs_experimenter.end()) {
    return;
  }
  mbs_experimenter.erase(index);
...
cofmeter_band_experimenter &
cofmeter_bands::set_meter_band_experimenter(unsigned int index) {
  if (mbs_experimenter.find(index) == mbs_experimenter.end()) {
...
cofmeter_bands::get_meter_band_experimenter(unsigned int index) const {
  if (mbs_experimenter.find(index) == mbs_experimenter.end()) {
    throw eRofMeterBandNotFound();
  }
...
bool cofmeter_bands::has_meter_band_experimenter(unsigned int index) const {
  return (not(mbs_experimenter.find(index) == mbs_experimenter.end()));
}

size_t cofmeter_bands::length() const {
  switch (of_version) {
  case openflow13::OFP_VERSION: {
    ...
    for (std::map<unsigned int, cofmeter_band_dscp_remark>::const_iterator it =
             mbs_dscp_remark.begin();
         it != mbs_dscp_remark.end(); ++it) {
      len += it->second.length();
    }
    for (std::map<unsigned int, cofmeter_band_experimenter>::const_iterator it =
             mbs_experimenter.begin();
         it != mbs_experimenter.end(); ++it) {
      len += it->second.length();
    }
    return len;
  } break;
...
void cofmeter_bands::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
  ...
  case rofl::openflow13::OFP_VERSION: {
    for (std::map<unsigned int, cofmeter_band_drop>::iterator it =
             mbs_drop.begin();
         it != mbs_drop.end(); ++it) {
    ...
    for (std::map<unsigned int, cofmeter_band_dscp_remark>::iterator it =
             mbs_dscp_remark.begin();
         it != mbs_dscp_remark.end(); ++it) {
      cofmeter_band &mb = it->second;
      mb.pack(buf + offset, mb.length());
    ...
    for (std::map<unsigned int, cofmeter_band_experimenter>::iterator it =
             mbs_experimenter.begin();
         it != mbs_experimenter.end(); ++it) {
      cofmeter_band &mb = it->second;
      mb.pack(buf + offset, mb.length());
...
void cofmeter_bands::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
  ...
  case rofl::openflow13::OFP_VERSION: {
    ...
    while (offset < buflen) {
      ...
      if (((buflen - offset) <
           sizeof(struct rofl::openflow13::ofp_meter_band_header)) ||
          (be16toh(mbh->len) <
           sizeof(struct rofl::openflow13::ofp_meter_band_header)))
        return;
      ...
      case rofl::openflow13::OFPMBT_EXPERIMENTER: {
        add_meter_band_experimenter(index_experimenter)
            .unpack(buf + offset, len);
        offset += len;
        index_experimenter++;
      } break;
      default:
        std::cerr << "[rofl][cofmeterbands][unpack] unknown meter band type:"
                  << (int)type << std::endl;
        offset += len;
      }
    }
