...
#define OXM_TLV_OFB_IN_PORT                                                    \
  ((OFPXMC_OPENFLOW_BASIC << 16) + (OFPXMT_OFB_IN_PORT << 9) +                 \
   OXM_TLV_PAYLOAD_4BYTES)
#define OXM_TLV_OFB_IN_PHY_PORT                                                \
  ((OFPXMC_OPENFLOW_BASIC << 16) + (OFPXMT_OFB_IN_PHY_PORT << 9) +             \
   OXM_TLV_PAYLOAD_4BYTES)
#define OXM_TLV_OFB_METADATA                                                   \
  ((OFPXMC_OPENFLOW_BASIC << 16) + (OFPXMT_OFB_METADATA << 9) +                \
   OXM_TLV_PAYLOAD_8BYTES)
#define OXM_TLV_OFB_METADATA_MASK                                              \
  ((OFPXMC_OPENFLOW_BASIC << 16) + (OFPXMT_OFB_METADATA << 9) +                \
   2 * OXM_TLV_PAYLOAD_8BYTES + OXM_TLV_HAS_MASK)
#define OXM_TLV_OFB_ETH_DST                                                    \
  ((OFPXMC_OPENFLOW_BASIC << 16) + (OFPXMT_OFB_ETH_DST << 9) +                 \
   OXM_TLV_PAYLOAD_6BYTES)
#define OXM_TLV_OFB_ETH_DST_MASK                                               \
  ((OFPXMC_OPENFLOW_BASIC << 16) + (OFPXMT_OFB_ETH_DST << 9) +                 \
   2 * OXM_TLV_PAYLOAD_6BYTES + OXM_TLV_HAS_MASK)
#define OXM_TLV_OFB_ETH_SRC                                                    \
  ((OFPXMC_OPENFLOW_BASIC << 16) + (OFPXMT_OFB_ETH_SRC << 9) +                 \
   OXM_TLV_PAYLOAD_6BYTES)
#define OXM_TLV_OFB_ETH_SRC_MASK                                               \
  ((OFPXMC_OPENFLOW_BASIC << 16) + (OFPXMT_OFB_ETH_SRC << 9) +                 \
   2 * OXM_TLV_PAYLOAD_6BYTES + OXM_TLV_HAS_MASK)
#define OXM_TLV_OFB_ETH_TYPE                                                   \
  ((OFPXMC_OPENFLOW_BASIC << 16) + (OFPXMT_OFB_ETH_TYPE << 9) +                \
   OXM_TLV_PAYLOAD_2BYTES)
// to be continued ...
...
enum ofp_flow_mod_flags {
  OFPFF_SEND_FLOW_REM =
      1 << 0, /* Send flow removed message when flow expires or is deleted. */
...
static int const OFP_FLOW_MOD_STATIC_HDR_LEN =
    sizeof(struct ofp_flow_mod) - sizeof(struct ofp_match);
...
struct ofp_flow_stats_request {
  uint8_t table_id; /* ID of table to read (from ofp_table_stats), OFPTT_ALL for
                       all tables. */
  uint8_t pad[3];   /* Align to 32 bits. */
  uint32_t out_port;  /* Require matching entries to include this as an output
                         port. A value of OFPP_ANY indicates no restriction. */
  uint32_t out_group; /* Require matching entries to include this as an output
  ...
  uint64_t cookie_mask;   /* Mask used to restrict the cookie bits that must
...
struct ofp_table_stats {
  uint8_t table_id; /* Identifier of table. Lower numbered tables are consulted
                       first. */
  uint8_t pad[3];   /* Align to 32-bits. */
...
struct ofp_table_feature_prop_instructions {
  ...
  uint16_t length; /* Length in bytes of this property. */
                    ...
                    *   bytes of all-zero bytes */
  struct ofp_instruction
      instruction_ids[0]; /* List of instructions: 4 bytes per instruction
...
struct ofp_table_feature_prop_actions {
  uint16_t type;   /* One of OFPTFPT_WRITE_ACTIONS, OFPTFPT_WRITE_ACTIONS_MISS,
...
struct ofp_table_feature_prop_oxm {
  uint16_t type;       /* One of OFPTFPT_MATCH, OFPTFPT_WILDCARDS,
...
struct ofp_port_stats {
  ...
  uint64_t rx_errors;     /* Number of receive errors. This is a super-set
                                             of more specific receive errors and
                             should be
                                             greater than or equal to the sum of all
                                             rx_*_err values. */
  uint64_t tx_errors;     /* Number of transmit errors. This is a super-set
                                             of more specific transmit errors and
                             should be
                                             greater than or equal to the sum of all
                                             tx_*_err values (none currently
                             defined.) */
...
struct ofp_group_stats {
  ...
  uint8_t pad[2];     /* Align to 64 bits. */
  uint32_t group_id;  /* Group identifier. */
  uint32_t ref_count; /* Number of flows or groups that directly forward to this
                         group. */
  uint8_t pad2[4];    /* Align to 64 bits. */
...
struct ofp_meter_stats {
  ...
  uint32_t duration_nsec;   /* Time meter has been alive in nanoseconds beyond
                               duration_sec. */
  struct ofp_meter_band_stats
      band_stats[0]; /* The band_stats length is inferred from the length field.
                      */
};
...
// for request, see structure ofp_meter_multipart_request; in section 7.3.5.12
// "Meter Statistics" above
...
struct ofp_meter_config {
  ...
  uint32_t meter_id; /* Meter instance. */
  struct ofp_meter_band_header
      bands[0]; /* The bands length is inferred from the length field. */
};
