...
class eTableModPropBase : public exception {
public:
  eTableModPropBase(const std::string &__arg) : exception(__arg){};
};
...
public:
  ...
  coftablemod_prop(
      ...
      : ofp_version(of_version), type(type), len(len){};

  /**
   *
   */
  virtual ~coftablemod_prop(){};

  /**
   *
   */
  coftablemod_prop(const coftablemod_prop &prop) { *this = prop; };

  /**
   *
   */
  coftablemod_prop &operator=(const coftablemod_prop &pdp) {
    if (this == &pdp)
      return *this;
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  coftablemod_prop const &prop) {
    os << "<coftablemod_prop version: " << (int)prop.get_version() << std::hex
...
public:
  ...
  coftablemod_prop_eviction(
      ...
      : coftablemod_prop(
            ofp_version, rofl::openflow14::OFPTMPT_EVICTION,
   ...
   *
   */
  virtual ~coftablemod_prop_eviction(){};
   ...
   *
   */
  coftablemod_prop_eviction &operator=(const coftablemod_prop_eviction &prop) {
    if (this == &prop)
      return *this;
...
public:
  friend std::ostream &operator<<(std::ostream &os,
    ...
    os << "<coftablemod_prop_eviction version: "
       << (unsigned int)prop.get_version() << " >" << std::endl;
    { os << dynamic_cast<const coftablemod_prop &>(prop); }
    os << "<flags: 0x" << std::hex << (unsigned int)prop.get_flags() << std::dec
       << " >" << std::endl;
    return os;
  };
...
public:
  ...
  coftablemod_prop_vacancy(
      uint8_t ofp_version = rofl::openflow::OFP_VERSION_UNKNOWN)
      : coftablemod_prop(
            ofp_version, rofl::openflow14::OFPTMPT_VACANCY,
            sizeof(struct rofl::openflow14::ofp_table_mod_prop_vacancy)),
        vacancy_down(0), vacancy_up(0), vacancy(0){};

  /**
   *
   */
  virtual ~coftablemod_prop_vacancy(){};
   ...
   *
   */
  coftablemod_prop_vacancy &operator=(const coftablemod_prop_vacancy &prop) {
    if (this == &prop)
      return *this;
    coftablemod_prop::operator=(prop);
...
public:
   ...
   *
   */
  uint8_t get_vacancy_down() const { return vacancy_down; };

  /**
   *
   */
  void set_vacancy_down(uint8_t vacancy_down) {
   ...
   *
   */
  uint8_t get_vacancy_up() const { return vacancy_up; };

  /**
   *
   */
  void set_vacancy_up(uint8_t vacancy_up) { this->vacancy_up = vacancy_up; };

  /**
   *
   */
  uint8_t get_vacancy() const { return vacancy; };

  /**
   *
   */
  void set_vacancy(uint8_t vacancy) { this->vacancy = vacancy; };

public:
  friend std::ostream &operator<<(std::ostream &os,
                                  const coftablemod_prop_vacancy &prop) {
    os << "<coftablemod_prop_vacancy version: "
       << (unsigned int)prop.get_version() << " >" << std::endl;
    { os << dynamic_cast<const coftablemod_prop &>(prop); }
    os << "<vacancy_down: 0x" << std::hex
       << (unsigned int)prop.get_vacancy_down() << std::dec << " >"
       << std::endl;
    os << "<vacancy_up: 0x" << std::hex << (unsigned int)prop.get_vacancy_up()
       << std::dec << " >" << std::endl;
    os << "<vacancy: 0x" << std::hex << (unsigned int)prop.get_vacancy()
       << std::dec << " >" << std::endl;
    return os;
  };
...
public:
  ...
  coftablemod_prop_experimenter(
      uint8_t ofp_version = rofl::openflow::OFP_VERSION_UNKNOWN)
      : coftablemod_prop(
            ofp_version, rofl::openflow14::OFPPDPT_EXPERIMENTER,
   ...
   *
   */
  virtual ~coftablemod_prop_experimenter(){};
   ...
   */
  coftablemod_prop_experimenter &
  operator=(const coftablemod_prop_experimenter &prop) {
    if (this == &prop)
      return *this;
    coftablemod_prop::operator=(prop);
    exp_id = prop.exp_id;
...
public:
  ...
  virtual size_t length() const {
    switch (get_version()) {
    case rofl::openflow14::OFP_VERSION: {
      return (sizeof(struct rofl::openflow14::ofp_table_mod_prop_experimenter) +
              exp_body.memlen());
    } break;
    default:
      throw eBadVersion("eBadVersion", __FILE__, __PRETTY_FUNCTION__, __LINE__);
...
public:
  friend std::ostream &operator<<(std::ostream &os,
    ...
    os << "<coftablemod_prop_experimenter version: "
       << (unsigned int)prop.get_version() << " >" << std::endl;
    { os << dynamic_cast<const coftablemod_prop &>(prop); }
