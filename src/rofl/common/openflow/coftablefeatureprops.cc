...
coftable_feature_props &coftable_feature_props::
operator=(coftable_feature_props const &tfps) {
  if (this == &tfps)
    return *this;
  ...
  for (std::map<uint16_t, coftable_feature_prop *>::const_iterator it =
    ...
    case rofl::openflow13::OFPTFPT_INSTRUCTIONS_MISS: {
      tfprops[type] = new coftable_feature_prop_instructions(
          *dynamic_cast<coftable_feature_prop_instructions const *>(
              it->second));
    } break;
    case rofl::openflow13::OFPTFPT_NEXT_TABLES:
    case rofl::openflow13::OFPTFPT_NEXT_TABLES_MISS: {
      tfprops[type] = new coftable_feature_prop_next_tables(
          *dynamic_cast<coftable_feature_prop_next_tables const *>(it->second));
    } break;
    ...
    case rofl::openflow13::OFPTFPT_APPLY_ACTIONS_MISS: {
      tfprops[type] = new coftable_feature_prop_actions(
          *dynamic_cast<coftable_feature_prop_actions const *>(it->second));
    } break;
    ...
    case rofl::openflow13::OFPTFPT_APPLY_SETFIELD_MISS: {
      tfprops[type] = new coftable_feature_prop_oxm(
          *dynamic_cast<coftable_feature_prop_oxm const *>(it->second));
    } break;
    default: {
      tfprops[type] = new coftable_feature_prop(
          *dynamic_cast<coftable_feature_prop const *>(it->second));
    } break;
    }
  }
...
void coftable_feature_props::clear() {
  for (std::map<uint16_t, coftable_feature_prop *>::iterator it =
           tfprops.begin();
       it != tfprops.end(); ++it) {
    delete it->second;
  }
...
size_t coftable_feature_props::length() const {
  size_t len = 0;
  for (std::map<uint16_t, coftable_feature_prop *>::const_iterator it =
           tfprops.begin();
       it != tfprops.end(); ++it) {
...
void coftable_feature_props::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen)) {
    return;
  }
  ...
  for (std::map<uint16_t, coftable_feature_prop *>::iterator it =
    ...
    if (sizeof(struct openflow13::ofp_table_feature_prop_header) > prop_len) {
      std::cerr << "[rofl][table-feature-props] internal error, tfp with "
                   "invalid length"
                << std::endl;
      assert(sizeof(struct openflow13::ofp_table_feature_prop_header) <=
             prop_len);
      return;
    }
...
void coftable_feature_props::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen)) {
    return;
  }
  ...
  while (buflen > 0) {
    struct openflow13::ofp_table_feature_prop_header *prop =
        (struct openflow13::ofp_table_feature_prop_header *)buf;

    if (buflen < sizeof(struct openflow13::ofp_table_feature_prop_header)) {
      return; // ignore padding
    }

    if (be16toh(prop->length) <
    ...
    size_t total_length =
        be16toh(prop->length) +
        ((0x7 & be16toh(prop->length)) ? 8 - (0x7 & be16toh(prop->length)) : 0);
    ...
    case rofl::openflow13::OFPTFPT_APPLY_ACTIONS:
    case rofl::openflow13::OFPTFPT_APPLY_ACTIONS_MISS: {
      (tfprops[be16toh(prop->type)] =
           new coftable_feature_prop_actions(ofp_version, be16toh(prop->type)))
          ->unpack(buf, total_length);
    } break;
    case rofl::openflow13::OFPTFPT_MATCH:
    case rofl::openflow13::OFPTFPT_WILDCARDS:
...
coftable_feature_prop &coftable_feature_props::add_tfp(unsigned int type) {
...
coftable_feature_prop &coftable_feature_props::set_tfp(unsigned int type) {
  if (tfprops.find(type) == tfprops.end()) {
    switch (type) {
    case rofl::openflow13::OFPTFPT_INSTRUCTIONS:
    case rofl::openflow13::OFPTFPT_INSTRUCTIONS_MISS: {
...
void coftable_feature_props::drop_tfp(unsigned int type) {
  if (tfprops.find(type) == tfprops.end()) {
    return;
  }
  delete tfprops[type];
...
bool coftable_feature_props::has_tfp(unsigned int type) const {
