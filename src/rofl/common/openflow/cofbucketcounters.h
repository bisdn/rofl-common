...
public:
	...
	get_bucket_counter(
			uint32_t bucket_counter_id) const {
	 ...
	 */
	bool
	has_bucket_counter(uint32_t bucket_counter_id) const {
		AcquireReadLock rwlock(bcs_lock);
...
public:
	 ...
	 */
	friend std::ostream&
	operator<< (std::ostream& os, const cofbucket_counters& bcs) {
