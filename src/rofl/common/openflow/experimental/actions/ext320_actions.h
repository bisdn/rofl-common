...
struct onf_act_copy_field_hdr {
  ...
  uint8_t data[0]; /* Source and destination OXM ids in any f...ing weird
                      combination. Who was so dumb to define this s...? */
...
public:
   ...
   *
   */
  cofaction_body_copy_field(uint16_t n_bits = 0, uint16_t src_offset = 0,
                            uint16_t dst_offset = 0, uint32_t src_oxm_id = 0,
                            uint32_t src_oxm_exp_id = 0,
                            uint32_t dst_oxm_id = 0,
                            uint32_t dst_oxm_exp_id = 0) {
    size_t memlen =
        sizeof(struct onf_act_copy_field_hdr) +
    ...
    if (memlen % sizeof(uint64_t))
      memlen += sizeof(uint64_t) -
                (memlen % sizeof(uint64_t)); // padding to 8 bytes boundary
...
public:
  ...
  uint32_t get_src_oxm_id() const {
    if (rofl::cmemory::memlen() < sizeof(struct onf_act_copy_field_hdr))
      throw eInvalid("cofaction_body_copy_field::get_src_oxm_id() too short, "
                     "no header found")
          .set_func(__FUNCTION__)
          .set_line(__LINE__);

    uint8_t *data =
        ((struct onf_act_copy_field_hdr *)rofl::cmemory::somem())->data;
    size_t datalen =
        rofl::cmemory::memlen() - sizeof(struct onf_act_copy_field_hdr);
    ...
    default: {
      throw eInvalid("cofaction_body_copy_field::get_src_oxm_id() unsupported "
                     "src-oxm-id class")
          .set_func(__FUNCTION__)
          .set_line(__LINE__);
    }
  ...
  uint32_t get_src_oxm_exp_id() const {
    if (rofl::cmemory::memlen() < sizeof(struct onf_act_copy_field_hdr))
      throw eInvalid(
          "cofaction_body_copy_field::get_src_oxm_exp_id() too short")
          .set_func(__FUNCTION__)
          .set_line(__LINE__);
    ...
    if (datalen < sizeof(uint32_t))
      throw eInvalid("cofaction_body_copy_field::get_src_oxm_exp_id() too "
                     "short, no src_oxm_id found")
          .set_func(__FUNCTION__)
          .set_line(__LINE__);
    ...
    case OFPXMC_EXPERIMENTER: {
      if (datalen < sizeof(uint64_t))
        throw eInvalid("cofaction_body_copy_field::get_src_oxm_exp_id() too "
                       "short, no src_oxm_id experimenter id found")
            .set_func(__FUNCTION__)
            .set_line(__LINE__);
  ...
  uint32_t get_dst_oxm_id() const {
    if (rofl::cmemory::memlen() < sizeof(struct onf_act_copy_field_hdr))
      throw eInvalid("cofaction_body_copy_field::get_dst_oxm_id() too short, "
                     "no header found")
          .set_func(__FUNCTION__)
          .set_line(__LINE__);
    ...
    if (src_oxm_exp_id > 0) {
      if (datalen <
          (2 * sizeof(uint32_t) + sizeof(uint32_t) /* 3rd 32bits */)) {
        throw eInvalid("cofaction_body_copy_field::get_dst_oxm_id() too short, "
                       "no dst_oxm_id found")
            .set_func(__FUNCTION__)
            .set_line(__LINE__);
    ...
    } else if (src_oxm_exp_id == 0) {
      if (datalen <
          (1 * sizeof(uint32_t) + sizeof(uint32_t) /* 2nd 32bits */)) {
        throw eInvalid("cofaction_body_copy_field::get_dst_oxm_id() too short, "
                       "no dst_oxm_id found")
            .set_func(__FUNCTION__)
            .set_line(__LINE__);
    ...
    default: {
      throw eInvalid("cofaction_body_copy_field::get_dst_oxm_id() unsupported "
                     "dst-oxm-id class")
          .set_func(__FUNCTION__)
          .set_line(__LINE__);
    }
  ...
  uint32_t get_dst_oxm_exp_id() const {
    if (rofl::cmemory::memlen() < sizeof(struct onf_act_copy_field_hdr))
      throw eInvalid("cofaction_body_copy_field::get_dst_oxm_exp_id() too "
                     "short, no header found")
          .set_func(__FUNCTION__)
          .set_line(__LINE__);
    ...
    case OFPXMC_EXPERIMENTER: {
      if (datalen < sizeof(uint64_t))
        throw eInvalid("cofaction_body_copy_field::get_src_oxm_exp_id() too "
                       "short, no dst_oxm_id experimenter id found")
            .set_func(__FUNCTION__)
            .set_line(__LINE__);
      return be32toh(*((uint32_t *)(data + sizeof(uint32_t))));
    } break;
   ...
   *
   */
  void set_oxm_ids(uint32_t src_oxm_id = 0,
                   uint32_t src_oxm_exp_id =
                       0 /* should be 0 for non-experimental oxm_ids */,
                   uint32_t dst_oxm_id = 0,
                   uint32_t dst_oxm_exp_id =
                       0 /* should be 0 for non-experimental oxm_ids */) {
    if (rofl::cmemory::memlen() < sizeof(struct onf_act_copy_field_hdr))
      throw eInvalid("cofaction_body_copy_field::set_src_offset()")
          .set_func(__FUNCTION__)
    ...
    if (src_oxm_exp_id > 0) {
      if (datalen < (offset + sizeof(uint64_t))) {
        throw eInvalid("cofaction_body_copy_field::set_oxm_id() too short, not "
                       "enough space for storing 64bit src_oxm_id")
            .set_func(__FUNCTION__)
            .set_line(__LINE__);
      }

      *(uint32_t *)(data + offset + 0 * sizeof(uint32_t)) = htobe32(src_oxm_id);
      *(uint32_t *)(data + offset + 1 * sizeof(uint32_t)) =
      ...
      if (datalen < sizeof(uint64_t)) {
        throw eInvalid("cofaction_body_copy_field::set_oxm_id() too short, not "
                       "enough space for storing 32bit src_oxm_id")
            .set_func(__FUNCTION__)
            .set_line(__LINE__);
      }
    ...
    if (dst_oxm_exp_id > 0) {
      if (datalen < (offset + sizeof(uint64_t))) {
        throw eInvalid("cofaction_body_copy_field::set_oxm_id() too short, not "
                       "enough space for storing 64bit dst_oxm_id")
            .set_func(__FUNCTION__)
            .set_line(__LINE__);
      }
      ...
      *(uint32_t *)(data + offset + 1 * sizeof(uint32_t)) =
          htobe32(dst_oxm_exp_id);

      offset = 2 * sizeof(uint32_t);
    } else if (dst_oxm_exp_id == 0) {
      if (datalen < sizeof(uint64_t)) {
        throw eInvalid("cofaction_body_copy_field::set_oxm_id() too short, not "
                       "enough space for storing 32bit dst_oxm_id")
            .set_func(__FUNCTION__)
            .set_line(__LINE__);
      }
