...
public:
  eFlowStatsBase(const std::string &__arg = std::string(""))
      : exception(__arg) {
...
public:
  eFlowStatsNotFound(const std::string &__arg = std::string(""))
...
public:
   ...
   *
   */
  cofflow_stats_request(uint8_t of_version, const cofmatch &match,
                        uint8_t table_id = 0xff,
                        uint32_t out_port = rofl::openflow::OFPP_ANY,
                        uint32_t out_group = rofl::openflow::OFPG_ANY,
                        uint64_t cookie = 0, uint64_t cookie_mask = 0)
      : of_version(of_version), match(match), table_id(table_id),
        out_port(out_port), out_group(out_group), cookie(cookie),
   ...
   *
   */
  cofflow_stats_request(const cofflow_stats_request &request) {
    *this = request;
  };
   ...
   *
   */
  cofflow_stats_request &operator=(const cofflow_stats_request &request) {
    if (this == &request)
      return *this;
...
public:
   ...
   *
   */
  uint32_t get_out_port() const { return out_port; };
   ...
   *
   */
  uint32_t get_out_group() const { return out_group; };
   ...
   *
   */
  uint64_t get_cookie_mask() const { return cookie_mask; };

  /**
   *
   */
  cofflow_stats_request &set_match(const cofmatch &match) {
    (this->match = match).set_version(of_version);
    return *this;
  };
   ...
   *
   */
  cofmatch &set_match() { return match; };

  /**
   *
   */
  const cofmatch &get_match() const { return match; };
...
public:
  ...
  operator<<(std::ostream &os,
    ...
    case openflow10::OFP_VERSION: {
      ...
      os << "<table-id:" << (int)flow_stats_request.get_table_id() << " >"
         << std::endl;
      os << "<out-port:0x" << std::hex << (int)flow_stats_request.get_out_port()
         << std::dec << " >" << std::endl;
    ...
    case openflow13::OFP_VERSION: {
      ...
      os << "<out-port:0x" << std::hex << (int)flow_stats_request.get_out_port()
         << std::dec << " >" << std::endl;
      os << "<out-group:0x" << std::hex
         << (int)flow_stats_request.get_out_group() << std::dec << " >"
         << std::endl;
      os << "<cookie:0x" << std::hex
         << (unsigned long long)flow_stats_request.get_cookie() << std::dec
         << " >" << std::endl;
      os << "<cookie-mask:0x" << std::hex
...
public:
  ...
  cofflow_stats_reply(uint8_t of_version = 0, uint8_t *buf = nullptr,
      ...
      : of_version(of_version), table_id(0), duration_sec(0), duration_nsec(0),
        ...
        packet_count(0), byte_count(0), match(of_version), actions(of_version),
   ...
   *
   */
  cofflow_stats_reply(uint8_t of_version, uint8_t table_id,
                      uint32_t duration_sec, uint32_t duration_nsec,
                      uint16_t priority, uint16_t idle_timeout,
                      uint16_t hard_timeout, uint64_t cookie,
                      uint64_t packet_count, uint64_t byte_count,
                      const cofmatch &match, const cofactions &actions)
      : of_version(of_version), table_id(table_id), duration_sec(duration_sec),
        duration_nsec(duration_nsec), priority(priority),
        idle_timeout(idle_timeout), hard_timeout(hard_timeout), flags(0),
        cookie(cookie), packet_count(packet_count), byte_count(byte_count),
  ...
  cofflow_stats_reply(uint8_t of_version, uint8_t table_id,
      ...
      : of_version(of_version), table_id(table_id), duration_sec(duration_sec),
        duration_nsec(duration_nsec), priority(priority),
        idle_timeout(idle_timeout), hard_timeout(hard_timeout), flags(flags),
        cookie(cookie), packet_count(packet_count), byte_count(byte_count),
   ...
   *
   */
  cofflow_stats_reply(const cofflow_stats_reply &reply) { *this = reply; };

  /**
   *
   */
  cofflow_stats_reply &operator=(const cofflow_stats_reply &reply) {
    if (this == &reply)
      return *this;
   ...
   *
   */
  bool operator==(const cofflow_stats_reply &reply) const {
    return ((of_version == reply.of_version) && (table_id == reply.table_id) &&
            (duration_sec == reply.duration_sec) &&
            (duration_nsec == reply.duration_nsec) &&
            (priority == reply.priority) &&
            (idle_timeout == reply.idle_timeout) &&
            (hard_timeout == reply.hard_timeout) && (flags == reply.flags) &&
            (cookie == reply.cookie) && (packet_count == reply.packet_count) &&
            (byte_count == reply.byte_count) && (match == reply.match) &&
            (actions == reply.actions) && (instructions == reply.instructions));
  };
...
public:
   ...
   *
   */
  cofflow_stats_reply &set_match(const cofmatch &match) {
    (this->match = match).set_version(of_version);
    return *this;
  };
   ...
   *
   */
  cofflow_stats_reply &set_actions(const cofactions &actions) {
    (this->actions = actions).set_version(of_version);
    return *this;
  };
   ...
   *
   */
  cofflow_stats_reply &set_instructions(const cofinstructions &instructions) {
    (this->instructions = instructions).set_version(of_version);
    return *this;
  };
   ...
   *
   */
  cofinstructions &set_instructions() { return instructions; };

  /**
   *
   */
  const cofinstructions &get_instructions() const { return instructions; };
...
public:
  friend std::ostream &operator<<(std::ostream &os,
    ...
    case rofl::openflow13::OFP_VERSION: {
      ...
      os << "<table-id: " << (int)flow_stats_reply.get_table_id() << " >"
         << std::endl;
      os << "<duration-sec: " << (int)flow_stats_reply.get_duration_sec()
         << " >" << std::endl;
      os << "<duration-nsec: " << (int)flow_stats_reply.get_duration_nsec()
         << " >" << std::endl;
      os << "<priority: 0x" << std::hex << (int)flow_stats_reply.get_priority()
         << std::dec << " >" << std::endl;
      os << "<idle-timeout: " << (int)flow_stats_reply.get_idle_timeout()
         << " >" << std::endl;
      os << "<hard-timeout: " << (int)flow_stats_reply.get_hard_timeout()
         << " >" << std::endl;
      os << "<cookie: 0x" << std::hex
         << (unsigned long long)flow_stats_reply.get_cookie() << std::dec
         << " >" << std::endl;
      os << "<packet-count: " << (int)flow_stats_reply.get_packet_count()
         << " >" << std::endl;
      os << "<byte-count: " << (int)flow_stats_reply.get_byte_count() << " >"
         << std::endl;
      os << "<flags: " << (int)flow_stats_reply.get_flags() << " >"
         << std::endl;
    ...
    switch (flow_stats_reply.of_version) {
    case openflow10::OFP_VERSION:
      os << flow_stats_reply.actions;
      break;
    case openflow12::OFP_VERSION:
      os << flow_stats_reply.instructions;
      break;
    case openflow13::OFP_VERSION:
      os << flow_stats_reply.instructions;
      break;
    default:
      os << "<unknown OFP version >" << std::endl;
    }
    return os;
  };
