		...
		const coftablemod_props& props)
{
	...
	for (std::map<uint16_t, coftablemod_prop*>::const_iterator
		...
		default:
			rofl::logging::warn << "[rofl][coftablemod_props] ignoring unknown port descrption property, property-type:"
		...
		uint8_t *buf, size_t buflen)
{
	...
	case openflow14::OFP_VERSION: {
		...
		while (buflen >= sizeof(struct rofl::openflow14::ofp_table_mod_prop_header)) {
			...
			default: {
				rofl::logging::warn << "[rofl][coftablemod_props] ignoring unknown port description property, property-type:"
