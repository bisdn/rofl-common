...
cofmeter_features_reply::cofmeter_features_reply(uint8_t of_version,
                                                 uint8_t *buf, size_t buflen)
    : of_version(of_version), max_meter(0), band_types(0), capabilities(0),
      max_bands(0), max_color(0) {
  if ((buflen > 0) && (0 != buf)) {
    unpack(buf, buflen);
  }
}
...
cofmeter_features_reply::cofmeter_features_reply(
    uint8_t of_version, uint32_t max_meter, uint32_t band_types,
    uint32_t capabilities, uint8_t max_bands, uint8_t max_color)
    : of_version(of_version), max_meter(max_meter), band_types(band_types),
      capabilities(capabilities), max_bands(max_bands), max_color(max_color) {}
...
cofmeter_features_reply &cofmeter_features_reply::
operator=(const cofmeter_features_reply &reply) {
  if (this == &reply)
    return *this;
...
bool cofmeter_features_reply::operator==(cofmeter_features_reply const &reply) {
  return ((of_version == reply.of_version) && (max_meter == reply.max_meter) &&
          (band_types == reply.band_types) &&
          (capabilities == reply.capabilities) &&
          (max_bands == reply.max_bands) && (max_color == reply.max_color));
}

void cofmeter_features_reply::clear() {
  max_meter = 0;
  band_types = 0;
  capabilities = 0;
  max_bands = 0;
...
size_t cofmeter_features_reply::length() const {
  switch (of_version) {
...
void cofmeter_features_reply::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
...
void cofmeter_features_reply::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
