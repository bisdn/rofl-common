...
public:
   ...
   *
   */
  cofportstatsarray(const cofportstatsarray &ports) { *this = ports; };

  /**
   *
   */
  cofportstatsarray &operator=(const cofportstatsarray &ports) {
    if (this == &ports)
      return *this;
   ...
   *
   */
  bool operator==(const cofportstatsarray &ports) const {
...
public:
   ...
   *
   */
  cofportstatsarray &set_version(uint8_t ofp_version) {
    AcquireReadLock lock(array_lock);
...
public:
   ...
   *
   */
  const cofport_stats_reply &get_port_stats(uint32_t port_id) const {
    AcquireReadLock lock(array_lock);
    if (array.find(port_id) == array.end()) {
   ...
   *
   */
  bool drop_port_stats(uint32_t port_id) {
    AcquireReadWriteLock lock(array_lock);
    if (array.find(port_id) == array.end()) {
      return false;
    }
   ...
   *
   */
  bool has_port_stats(uint32_t port_id) const {
    AcquireReadLock lock(array_lock);
...
public:
  friend std::ostream &operator<<(std::ostream &os,
    ...
    os << "<cofportstatsarray #ports:" << (int)portstatsarray.array.size()
       << " >" << std::endl;
    AcquireReadLock lock(portstatsarray.array_lock);
    for (std::map<uint32_t, cofport_stats_reply>::const_iterator it =
             portstatsarray.array.begin();
         it != portstatsarray.array.end(); ++it) {
      os << it->second;
    }
    return os;
  }
