...
cofmeter_band_stats_array::cofmeter_band_stats_array(uint8_t of_version)
...
cofmeter_band_stats_array &cofmeter_band_stats_array::
operator=(const cofmeter_band_stats_array &mbs) {
  if (this == &mbs)
    return *this;
  ...
  for (std::map<unsigned int, cofmeter_band_stats>::const_iterator it =
           mbs.array.begin();
       it != mbs.array.end(); ++it) {
...
void cofmeter_band_stats_array::clear() { array.clear(); }
...
void cofmeter_band_stats_array::drop_mbstats(unsigned int index) {
  if (array.find(index) == array.end()) {
    return;
  }
  array.erase(index);
...
size_t cofmeter_band_stats_array::length() const {
  switch (of_version) {
...
void cofmeter_band_stats_array::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
  ...
  case rofl::openflow13::OFP_VERSION: {
    ...
    for (std::map<unsigned int, cofmeter_band_stats>::iterator it =
             array.begin();
         it != array.end(); ++it) {
...
void cofmeter_band_stats_array::unpack(uint8_t *buf, size_t buflen) {
  switch (of_version) {
  case rofl::openflow13::OFP_VERSION: {
    ...
    while ((buflen - offset) >=
           sizeof(struct rofl::openflow13::ofp_meter_band_stats)) {
      add_mbstats(index).unpack(
          buf + offset, sizeof(struct rofl::openflow13::ofp_meter_band_stats));
