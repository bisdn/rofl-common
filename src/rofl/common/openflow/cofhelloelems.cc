...
cofhelloelems::~cofhelloelems() { clear(); }

cofhelloelems::cofhelloelems() {}

cofhelloelems::cofhelloelems(const cofhelloelems &elems) { *this = elems; }

cofhelloelems &cofhelloelems::operator=(const cofhelloelems &elems) {
  if (this == &elems)
    return *this;
...
void cofhelloelems::clear() {
  for (std::map<uint8_t, cofhello_elem *>::const_iterator it = elems.begin();
       it != elems.end(); ++it) {
    delete it->second;
  }
  elems.clear();
...
size_t cofhelloelems::length() const {
  size_t len = 0;
  for (std::map<uint8_t, cofhello_elem *>::const_iterator it = elems.begin();
       it != elems.end(); ++it) {
...
void cofhelloelems::pack(uint8_t *buf, size_t buflen) {
...
void cofhelloelems::unpack(uint8_t *buf, size_t buflen) {
  clear();
...
cofhello_elem_versionbitmap &cofhelloelems::add_hello_elem_versionbitmap() {
  if (elems.find(rofl::openflow13::OFPHET_VERSIONBITMAP) != elems.end()) {
    delete elems[rofl::openflow13::OFPHET_VERSIONBITMAP];
  }
  elems[rofl::openflow13::OFPHET_VERSIONBITMAP] =
      new cofhello_elem_versionbitmap();
  return *dynamic_cast<cofhello_elem_versionbitmap *>(
      elems[rofl::openflow13::OFPHET_VERSIONBITMAP]);
}

cofhello_elem_versionbitmap &cofhelloelems::set_hello_elem_versionbitmap() {
  if (elems.find(rofl::openflow13::OFPHET_VERSIONBITMAP) == elems.end()) {
    elems[rofl::openflow13::OFPHET_VERSIONBITMAP] =
        new cofhello_elem_versionbitmap();
  }
...
cofhelloelems::get_hello_elem_versionbitmap() const {
  ...
  return *dynamic_cast<cofhello_elem_versionbitmap *>(
      elems.at(rofl::openflow13::OFPHET_VERSIONBITMAP));
}

void cofhelloelems::drop_hello_elem_versionbitmap() {
  if (elems.find(rofl::openflow13::OFPHET_VERSIONBITMAP) == elems.end()) {
    return;
  }
  delete elems[rofl::openflow13::OFPHET_VERSIONBITMAP];
...
bool cofhelloelems::has_hello_elem_versionbitmap() const {
