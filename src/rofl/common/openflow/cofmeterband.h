...
public:
  eRofMeterBandBase(const std::string &__arg = std::string(""))
      : exception(__arg) {
...
public:
  eRofMeterBandNotFound(const std::string &__arg = std::string(""))
...
public:
   ...
   *
   */
  uint8_t *resize(size_t len) { return body.resize(len); }
...
public:
   ...
   *
   */
  void set_version(uint8_t of_version) { this->of_version = of_version; };
   ...
   *
   */
  void set_rate(uint32_t rate) { this->rate = rate; };

  /**
   *
   */
  uint32_t get_burst_size() const { return burst_size; };

  /**
   *
   */
  void set_burst_size(uint32_t burst_size) { this->burst_size = burst_size; };
   ...
   *
   */
  rofl::cmemory const &get_body() const { return body; };

public:
  /**
   *
   */
  friend std::ostream &operator<<(std::ostream &os, cofmeter_band const &mb) {
    os << "<cofmeter_band " << std::hex << " type: 0x" << mb.get_type()
       << " len: 0x" << mb.get_length() << " rate: 0x" << mb.get_rate()
       << " burst_size: 0x" << mb.get_burst_size() << std::dec << " >"
       << std::endl;
    { os << mb.get_body(); }
    return os;
  };
...
class cofmeter_band_drop : public cofmeter_band {
public:
  /**
   *
   */
  cofmeter_band_drop(uint8_t of_version = rofl::openflow13::OFP_VERSION);

  /**
   *
   */
  virtual ~cofmeter_band_drop();

  /**
   *
   */
  cofmeter_band_drop(const cofmeter_band_drop &mb);

  /**
   *
   */
  cofmeter_band_drop &operator=(const cofmeter_band_drop &mb);
...
class cofmeter_band_dscp_remark : public cofmeter_band {
public:
  /**
   *
   */
   ...
   *
   */
  virtual ~cofmeter_band_dscp_remark();
   ...
   *
   */
  cofmeter_band_dscp_remark &operator=(const cofmeter_band_dscp_remark &mb);
...
public:
   ...
   *
   */
  uint8_t get_prec_level() const { return prec_level; };

  /**
   *
   */
  void set_prec_level(uint8_t prec_level) { this->prec_level = prec_level; };

public:
  friend std::ostream &operator<<(std::ostream &os,
                                  const cofmeter_band_dscp_remark &mb) {
    os << "<cofmeter_band_dscp_remark prec-level: 0x" << std::hex
       << (int)mb.get_prec_level() << std::dec << " >" << std::endl;
...
class cofmeter_band_experimenter : public cofmeter_band {
public:
  /**
   *
   */
   ...
   *
   */
  virtual ~cofmeter_band_experimenter();
...
public:
   ...
   *
   */
  cmemory const &get_exp_body() const { return exp_body; };
   ...
   *
   */
  cmemory const &get_body() const { return exp_body; };

  /**
   *
   */
  cmemory &set_body() { return exp_body; };

public:
  friend std::ostream &operator<<(std::ostream &os,
                                  const cofmeter_band_experimenter &mb) {
    os << "<cofmeter_band_experimenter exp_id: 0x" << std::hex
       << (int)mb.get_exp_id() << std::dec << " >" << std::endl;
    { os << dynamic_cast<const cofmeter_band &>(mb); }
