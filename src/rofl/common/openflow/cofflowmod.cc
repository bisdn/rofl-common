...
void cofflowmod::check_prerequisites() const {
...
size_t cofflowmod::length() const {
  switch (get_version()) {
  case rofl::openflow10::OFP_VERSION: {
  ...
  case rofl::openflow12::OFP_VERSION:
  case rofl::openflow13::OFP_VERSION: {
    return (sizeof(struct ofp13_flow_mod) + match.length() +
            instructions.length());
  } break;
  default:
    throw eBadVersion("eBadVersion", __FILE__, __PRETTY_FUNCTION__, __LINE__);
...
void cofflowmod::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;

  if (buflen < length())
    throw eInvalid("cofflowmod::pack() buflen too short", __FILE__,
...
void cofflowmod::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
  ...
  case rofl::openflow10::OFP_VERSION: {
    ...
    if (buflen < sizeof(struct ofp10_flow_mod))
      throw eInvalid("cofflowmod::unpack() buflen too short", __FILE__,
                     __PRETTY_FUNCTION__, __LINE__);
  ...
  case rofl::openflow13::OFP_VERSION: {
    ...
    struct rofl::openflow13::ofp_match *m =
        (struct rofl::openflow13::ofp_match *)(hdr->match);
    ...
    size_t instructionslen =
        buflen - sizeof(struct ofp13_flow_mod) - match.length();
