...
cofmsg_group_features_stats_request::cofmsg_group_features_stats_request(
		...
		uint32_t xid,
		uint16_t flags) :
				cofmsg_stats_request(version, xid, rofl::openflow::OFPMP_GROUP_FEATURES, flags)
{}
...
void
cofmsg_group_features_stats_request::pack(
		uint8_t *buf, size_t buflen)
{
	...
	if (buflen < cofmsg_group_features_stats_request::length())
		throw eMsgInval("cofmsg_group_features_stats_request::pack() buf too short");
}



void
cofmsg_group_features_stats_request::unpack(
		uint8_t *buf, size_t buflen)
{
	...
	if (buflen < cofmsg_group_features_stats_request::length())
		throw eBadRequestBadLen("cofmsg_group_features_stats_request::unpack() buf too short");
...
cofmsg_group_features_stats_reply::cofmsg_group_features_stats_reply(
		...
		const rofl::openflow::cofgroup_features_stats_reply& group_features_stats) :
				cofmsg_stats_reply(version, xid, rofl::openflow::OFPMP_GROUP_FEATURES, stats_flags),
...
void
cofmsg_group_features_stats_reply::pack(
		uint8_t *buf, size_t buflen)
{
	...
	if (buflen < cofmsg_group_features_stats_reply::length())
		throw eMsgInval("cofmsg_group_features_stats_reply::pack() buf too short");
...
void
cofmsg_group_features_stats_reply::unpack(
		uint8_t *buf, size_t buflen)
{
	...
	if (buflen < cofmsg_group_features_stats_reply::length())
		throw eBadRequestBadLen("cofmsg_group_features_stats_reply::unpack() buf too short");
	...
	default: {
		...
		if (buflen > sizeof(struct rofl::openflow13::ofp_multipart_reply)) {
			group_features_stats.unpack(hdr->body, buflen - sizeof(struct rofl::openflow13::ofp_multipart_reply));
		}
	};
	}
