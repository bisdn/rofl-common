...
public:
   ...
   *
   */
  cofmsg_packet_in &operator=(const cofmsg_packet_in &msg) {
    if (this == &msg)
      return *this;
    cofmsg::operator=(msg);
...
public:
   ...
   *
   */
  uint16_t get_total_len() const { return total_len; };

  /**
   *
   */
  void set_total_len(uint16_t total_len) { this->total_len = total_len; };

  /**
   *
   */
  uint16_t get_in_port() const { return in_port; };

  /**
   *
   */
  void set_in_port(uint16_t in_port) { this->in_port = in_port; };
...
public:
  friend std::ostream &operator<<(std::ostream &os,
    ...
    case openflow13::OFP_VERSION: {
      ...
      os << "<reason:" << msg.s_reason() << " >" << std::endl;
      os << "<table-id: " << (int)msg.get_table_id() << " >" << std::endl;
      os << "<cookie: 0x" << std::hex << (unsigned long long)msg.get_cookie()
         << std::dec << " >" << std::endl;
