...
cofmsg_meter_mod::cofmsg_meter_mod(
    uint8_t version, uint32_t xid, uint16_t command, uint16_t flags,
    uint32_t meter_id, const rofl::openflow::cofmeter_bands &meter_bands)
    : cofmsg(version, rofl::openflow::OFPT_METER_MOD, xid), command(command),
      flags(flags), meter_id(meter_id), meter_bands(meter_bands) {
...
size_t cofmsg_meter_mod::length() const {
  switch (get_version()) {
  case rofl::openflow13::OFP_VERSION: {
    return (sizeof(struct rofl::openflow13::ofp_meter_mod) +
            meter_bands.length());
  } break;
  default:
...
void cofmsg_meter_mod::pack(uint8_t *buf, size_t buflen) {
...
void cofmsg_meter_mod::unpack(uint8_t *buf, size_t buflen) {
  ...
  default: {
    ...
    if (buflen > sizeof(struct rofl::openflow13::ofp_meter_mod)) {
      meter_bands.unpack((uint8_t *)hdr->bands,
                         buflen -
