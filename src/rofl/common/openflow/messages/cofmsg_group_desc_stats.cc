...
cofmsg_group_desc_stats_request::cofmsg_group_desc_stats_request(
		...
		uint32_t xid,
		uint16_t flags) :
				cofmsg_stats_request(of_version, xid, rofl::openflow::OFPMP_GROUP_DESC, flags)
{}
...
void
cofmsg_group_desc_stats_request::pack(
		uint8_t *buf, size_t buflen)
{
	...
	if (buflen < cofmsg_group_desc_stats_request::length())
		throw eMsgInval("cofmsg_group_desc_stats_request::pack() buf too short");
}



void
cofmsg_group_desc_stats_request::unpack(
		uint8_t *buf, size_t buflen)
{
	...
	if (buflen < cofmsg_group_desc_stats_request::length())
		throw eBadSyntaxTooShort("cofmsg_group_desc_stats_request::unpack() buf too short");

	switch (get_version()) {
	default: {
		if (get_stats_type() != rofl::openflow13::OFPMP_GROUP_DESC)
			throw eMsgInval("cofmsg_group_desc_stats_request::unpack() invalid statistics type");
	};
	}
...
void
cofmsg_group_desc_stats_reply::pack(
		uint8_t *buf, size_t buflen)
{
	...
	if (buflen < cofmsg_group_desc_stats_reply::length())
		throw eMsgInval("cofmsg_group_desc_stats_reply::pack() buf too short");
...
void
cofmsg_group_desc_stats_reply::unpack(
		uint8_t *buf, size_t buflen)
{
	...
	if (buflen < cofmsg_group_desc_stats_reply::length())
		throw eBadSyntaxTooShort("cofmsg_group_desc_stats_reply::unpack() buf too short");

	switch (get_version()) {
	default: {
		if (get_stats_type() != rofl::openflow13::OFPMP_GROUP_DESC)
			throw eMsgInval("cofmsg_group_desc_stats_reply::unpack() invalid statistics type");

		struct rofl::openflow13::ofp_multipart_reply* hdr =
				(struct rofl::openflow13::ofp_multipart_reply*)buf;
		if (buflen > sizeof(struct rofl::openflow13::ofp_multipart_reply)) {
			groupdescstatsarray.unpack(hdr->body, buflen - sizeof(struct rofl::openflow13::ofp_multipart_reply));
		}
	};
	}
