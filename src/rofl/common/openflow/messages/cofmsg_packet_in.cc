...
const size_t rofl::openflow::cofmsg_packet_in::OFP12_PACKET_IN_STATIC_HDR_LEN =
    (sizeof(struct rofl::openflow12::ofp_packet_in) -
     sizeof(struct rofl::openflow12::ofp_match)); // length without struct
                                                  // openflow12::ofp_match
/*static*/
const size_t rofl::openflow::cofmsg_packet_in::OFP13_PACKET_IN_STATIC_HDR_LEN =
    (sizeof(struct rofl::openflow13::ofp_packet_in) -
     sizeof(struct rofl::openflow13::ofp_match)); // length without struct
                                                  // openflow13::ofp_match

size_t cofmsg_packet_in::length() const {
  switch (get_version()) {
  case rofl::openflow10::OFP_VERSION: {
    return (OFP10_PACKET_IN_STATIC_HDR_LEN + packet.length());

  } break;
  case rofl::openflow12::OFP_VERSION: {
    return (OFP12_PACKET_IN_STATIC_HDR_LEN + match.length() +
            2 /* bytes padding */
            + packet.length());
  } break;
  default: {
    return (OFP13_PACKET_IN_STATIC_HDR_LEN + match.length() +
            2 /* bytes padding */
            + packet.length());
  };
  }
...
void cofmsg_packet_in::pack(uint8_t *buf, size_t buflen) {
  ...
  case rofl::openflow12::OFP_VERSION: {
    ...
    match.pack((buf + rofl::openflow12::OFP_PACKET_IN_STATIC_HDR_LEN),
               match.length());

    if (not packet.empty()) {
      packet.pack(buf + rofl::openflow12::OFP_PACKET_IN_STATIC_HDR_LEN +
                      match.length() + 2,
                  packet.length());
    }
  ...
  default: {
    ...
    match.pack((buf + rofl::openflow13::OFP_PACKET_IN_STATIC_HDR_LEN),
               match.length());

    if (not packet.empty()) {
      packet.pack(buf + rofl::openflow13::OFP_PACKET_IN_STATIC_HDR_LEN +
                      match.length() + 2,
                  packet.length());
    }
  };
...
void cofmsg_packet_in::unpack(uint8_t *buf, size_t buflen) {
