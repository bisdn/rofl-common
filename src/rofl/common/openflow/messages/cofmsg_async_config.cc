...
size_t cofmsg_get_async_config_request::length() const {
  switch (get_version()) {
...
void cofmsg_get_async_config_request::pack(uint8_t *buf, size_t buflen) {
...
void cofmsg_get_async_config_request::unpack(uint8_t *buf, size_t buflen) {
...
size_t cofmsg_get_async_config_reply::length() const {
  switch (get_version()) {
  case rofl::openflow13::OFP_VERSION: {
    return (sizeof(struct rofl::openflow13::ofp_header) +
            async_config.length());
  } break;
  default:
    throw eBadVersion("eBadVersion").set_func(__FUNCTION__).set_line(__LINE__);
...
void cofmsg_get_async_config_reply::pack(uint8_t *buf, size_t buflen) {
  cofmsg::pack(buf, buflen);
  ...
  switch (get_version()) {
  default: {
    async_config.pack(buf + sizeof(struct rofl::openflow::ofp_header),
                      async_config.length());
  };
  }
}

void cofmsg_get_async_config_reply::unpack(uint8_t *buf, size_t buflen) {
  cofmsg::unpack(buf, buflen);
  ...
  default: {
    ...
    if (buflen > sizeof(struct rofl::openflow::ofp_header)) {
      async_config.unpack(buf + sizeof(struct rofl::openflow::ofp_header),
...
size_t cofmsg_set_async_config::length() const {
  switch (get_version()) {
  case rofl::openflow13::OFP_VERSION: {
    return (sizeof(struct rofl::openflow13::ofp_header) +
            async_config.length());
...
void cofmsg_set_async_config::pack(uint8_t *buf, size_t buflen) {
  cofmsg::pack(buf, buflen);
...
void cofmsg_set_async_config::unpack(uint8_t *buf, size_t buflen) {
  cofmsg::unpack(buf, buflen);
