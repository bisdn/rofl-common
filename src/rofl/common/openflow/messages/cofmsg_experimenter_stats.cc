...
cofmsg_experimenter_stats_request::length() const
{
	...
	case rofl::openflow10::OFP_VERSION: {
		return (sizeof(struct rofl::openflow10::ofp_stats_request) +
				sizeof(struct rofl::openflow10::ofp_vendor_stats_header) + exp_body.memlen());
	} break;
	default: {
		return (sizeof(struct rofl::openflow13::ofp_multipart_request) +
				sizeof(struct rofl::openflow13::ofp_experimenter_multipart_header) + exp_body.memlen());
	};
	}
	return 0;
}
		...
		uint8_t *buf, size_t buflen)
{
	...
	case rofl::openflow10::OFP_VERSION: {
		...
		exp_body.pack(hdr->body + sizeof(struct rofl::openflow10::ofp_vendor_stats_header), exp_body.length());
	} break;
	default: {
		...
		struct rofl::openflow13::ofp_experimenter_multipart_header* exp_hdr =
				(struct rofl::openflow13::ofp_experimenter_multipart_header*)(buf + sizeof(struct rofl::openflow13::ofp_multipart_request));
		exp_hdr->experimenter = htobe32(exp_id);
		exp_hdr->exp_type = htobe32(exp_type);
		exp_body.pack(hdr->body + sizeof(struct rofl::openflow13::ofp_experimenter_multipart_header), exp_body.length());
	};
	}
}
...
void
cofmsg_experimenter_stats_request::unpack(
		uint8_t *buf, size_t buflen)
{
	...
	case rofl::openflow10::OFP_VERSION: {
		if (get_type() != rofl::openflow10::OFPT_STATS_REQUEST)
			throw eMsgInval("cofmsg_experimenter_stats_request::unpack() invalid message type");

		if (get_stats_type() != rofl::openflow10::OFPST_VENDOR)
			throw eMsgInval("cofmsg_experimenter_stats_request::unpack() invalid statistics type");
		...
		if (buflen > (sizeof(struct rofl::openflow10::ofp_stats_request) +
			...
			exp_body.unpack(hdr->body + sizeof(struct rofl::openflow10::ofp_vendor_stats_header),
	...
	default: {
		...
		if (buflen > (sizeof(struct rofl::openflow13::ofp_multipart_request) +
			...
			exp_body.unpack(hdr->body + sizeof(struct rofl::openflow13::ofp_experimenter_multipart_header),
...
void
cofmsg_experimenter_stats_reply::pack(
		uint8_t *buf, size_t buflen)
{
	...
	default: {
		...
		struct rofl::openflow13::ofp_experimenter_multipart_header* exp_hdr =
				(struct rofl::openflow13::ofp_experimenter_multipart_header*)(buf + sizeof(struct rofl::openflow13::ofp_multipart_reply));
		exp_hdr->experimenter = htobe32(exp_id);
		exp_hdr->exp_type = htobe32(exp_type);
		exp_body.pack(hdr->body + sizeof(struct rofl::openflow13::ofp_experimenter_multipart_header), exp_body.length());
	};
...
void
cofmsg_experimenter_stats_reply::unpack(
		uint8_t *buf, size_t buflen)
{
	...
	case rofl::openflow10::OFP_VERSION: {
		if (get_type() != rofl::openflow10::OFPT_STATS_REPLY)
			throw eMsgInval("cofmsg_experimenter_stats_reply::unpack() invalid message type");

		if (get_stats_type() != rofl::openflow10::OFPST_VENDOR)
			throw eMsgInval("cofmsg_experimenter_stats_reply::unpack() invalid statistics type");
