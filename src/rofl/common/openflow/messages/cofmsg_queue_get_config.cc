...
cofmsg_queue_get_config_request::cofmsg_queue_get_config_request(
		...
		uint32_t xid,
		uint32_t portno) :
				cofmsg(version, rofl::openflow::OFPT_QUEUE_GET_CONFIG_REQUEST/*=22*/, xid),
				portno(portno)
{
	switch (version) {
...
void
cofmsg_queue_get_config_request::unpack(
		uint8_t *buf, size_t buflen)
{
	...
	if (buflen < cofmsg_queue_get_config_request::length())
		throw eBadSyntaxTooShort("cofmsg_queue_get_config_request::unpack() buf too short");
...
cofmsg_queue_get_config_reply::length() const
{
	switch (get_version()) {
	case rofl::openflow10::OFP_VERSION: {
		return (sizeof(struct rofl::openflow10::ofp_queue_get_config_reply) + queues.length());
	} break;
	case rofl::openflow12::OFP_VERSION: {
		return (sizeof(struct rofl::openflow12::ofp_queue_get_config_reply) + queues.length());
	} break;
	case rofl::openflow13::OFP_VERSION: {
		return (sizeof(struct rofl::openflow13::ofp_queue_get_config_reply) + queues.length());
	} break;
	default:
		throw eBadVersion();
	}
...
void
cofmsg_queue_get_config_reply::unpack(
		uint8_t *buf, size_t buflen)
{
	cofmsg::unpack(buf, buflen);
	...
	if (buflen < cofmsg_queue_get_config_reply::length())
		throw eBadSyntaxTooShort("cofmsg_queue_get_config_reply::unpack() buf too short");
	...
	case rofl::openflow10::OFP_VERSION: {
		...
		if (buflen > sizeof(struct rofl::openflow10::ofp_queue_get_config_reply)) {
			queues.unpack((uint8_t*)(hdr->queues), buflen - sizeof(struct rofl::openflow10::ofp_queue_get_config_reply));
		}
	} break;
	default: {
		...
		if (buflen > sizeof(struct rofl::openflow13::ofp_queue_get_config_reply)) {
			queues.unpack((uint8_t*)(hdr->queues), buflen - sizeof(struct rofl::openflow13::ofp_queue_get_config_reply));
		}
	};
	}
