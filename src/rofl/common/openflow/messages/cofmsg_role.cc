...
cofmsg_role_request::cofmsg_role_request(uint8_t version, uint32_t xid,
                                         const rofl::openflow::cofrole &role)
    : cofmsg(version, rofl::openflow::OFPT_ROLE_REQUEST, xid), role(role) {
...
size_t cofmsg_role_request::length() const {
  switch (get_version()) {
...
void cofmsg_role_request::pack(uint8_t *buf, size_t buflen) {
...
void cofmsg_role_request::unpack(uint8_t *buf, size_t buflen) {
  ...
  default: {
    ...
    if (buflen > sizeof(struct rofl::openflow13::ofp_header)) {
      role.unpack(buf + sizeof(struct rofl::openflow13::ofp_header),
                  buflen - sizeof(struct rofl::openflow13::ofp_header));
    }
  };
  }
...
cofmsg_role_reply::cofmsg_role_reply(uint8_t version, uint32_t xid,
                                     const rofl::openflow::cofrole &role)
    : cofmsg(version, rofl::openflow::OFPT_ROLE_REPLY, xid), role(role) {
  this->role.set_version(version);
}
...
size_t cofmsg_role_reply::length() const {
  switch (get_version()) {
  case rofl::openflow12::OFP_VERSION: {
    return (sizeof(struct rofl::openflow12::ofp_header) + role.length());
  } break;
...
void cofmsg_role_reply::pack(uint8_t *buf, size_t buflen) {
  cofmsg::pack(buf, buflen);
...
void cofmsg_role_reply::unpack(uint8_t *buf, size_t buflen) {
  cofmsg::unpack(buf, buflen);
