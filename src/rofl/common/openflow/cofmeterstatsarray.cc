...
void cofmeterstatsarray::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
  ...
  case rofl::openflow13::OFP_VERSION: {
    ...
    for (std::map<unsigned int, cofmeter_stats_reply>::iterator it =
             array.begin();
         it != array.end(); ++it) {
...
void cofmeterstatsarray::unpack(uint8_t *buf, size_t buflen) {
  array.clear();
