...
coxmatch::coxmatch(uint32_t oxm_id, uint32_t exp_id)
{
	if (OFPAT_EXPERIMENTER == ((oxm_id >> 16) & OFPAT_EXPERIMENTER)) {
		// is experimenter
		rofl::cmemory::resize(sizeof(struct ofp_oxm_experimenter_header) +
				(size_t)(oxm_id & 0x000000ff));
		set_oxm_exp_id(exp_id);
	} else {
		rofl::cmemory::resize(sizeof(struct ofp_oxm_hdr) +
				(size_t)(oxm_id & 0x000000ff));
	}
...
void
coxmatch::set_oxm_exp_id(uint32_t exp_id)
{
	if (memlen() < sizeof(struct ofp_oxm_experimenter_header)) {
		throw eOxmBadLen("coxmatch::set_oxm_exp_id() invalid length");
	}
...
uint32_t
coxmatch::get_oxm_exp_id() const
{
	if (memlen() < sizeof(struct ofp_oxm_experimenter_header)) {
		throw eOxmBadLen("coxmatch::get_oxm_exp_id() invalid length");
	}
		...
		uint32_t oxm_id, uint8_t value, enum coxmatch_bit_t bits)
{
	...
	default: {
		throw eOxmInval("coxmatch_8::coxmatch_8() invalid OXM width");
	};
	}
}
...
uint8_t
coxmatch_8::get_u8masked_value() const {
	if (memlen() < sizeof(struct rofl::openflow::ofp_oxm_hdr) + 2*sizeof(uint8_t))
		return get_u8value();
	struct rofl::openflow::ofp_oxm_ofb_uint8_t* oxm =
			(struct rofl::openflow::ofp_oxm_ofb_uint8_t*)somem();
 ...
 * 8 bits
 */
coxmatch_8_exp::coxmatch_8_exp(
		uint32_t oxm_id, uint32_t exp_id, uint8_t value, enum coxmatch_bit_t bits)
{
	switch (bits) {
	case COXMATCH_8BIT: {
		rofl::cmemory::resize(sizeof(struct ofp_oxm_experimenter_header) + 1*sizeof(uint8_t));
		set_oxm_id(oxm_id);
		set_oxm_exp_id(exp_id);
		set_u8value(value);
	} break;
	default: {
		throw eOxmInval("coxmatch_8_exp::coxmatch_8_exp() invalid OXM width");
	};
	}
}
...
coxmatch_8_exp::coxmatch_8_exp(
		uint32_t oxm_id, uint32_t exp_id, uint8_t value, uint8_t mask, enum coxmatch_bit_t bits)
{
	switch (bits) {
	case COXMATCH_8BIT: {
...
void
coxmatch_8_exp::set_u8value(uint8_t value) {
	if (memlen() < sizeof(struct ofp_oxm_experimenter_header) + sizeof(uint8_t)) {
		throw eOxmBadLen("coxmatch::set_u8value() invalid length");
	}
...
void
coxmatch_8_exp::set_u8mask(uint8_t mask) {
	if (memlen() < sizeof(struct ofp_oxm_experimenter_header) + 2*sizeof(uint8_t))
		throw eOxmBadLen("coxmatch::set_u8mask() invalid length");
	struct rofl::openflow::ofp_oxm_ofb_exp_uint8_t* oxm =
			(struct rofl::openflow::ofp_oxm_ofb_exp_uint8_t*)somem();
	oxm->mask = mask;
...
uint8_t
coxmatch_8_exp::get_u8value() const {
	if (memlen() < sizeof(struct ofp_oxm_experimenter_header) + sizeof(uint8_t))
		throw eOxmBadLen("coxmatch::get_u8value() invalid length");
	struct rofl::openflow::ofp_oxm_ofb_exp_uint8_t* oxm =
			(struct rofl::openflow::ofp_oxm_ofb_exp_uint8_t*)somem();
...
uint8_t
coxmatch_8_exp::get_u8mask() const {
	if (memlen() < sizeof(struct ofp_oxm_experimenter_header) + 2*sizeof(uint8_t))
		return 0xff;
	struct rofl::openflow::ofp_oxm_ofb_exp_uint8_t* oxm =
			(struct rofl::openflow::ofp_oxm_ofb_exp_uint8_t*)somem();
...
uint8_t
coxmatch_8_exp::get_u8masked_value() const {
	if (memlen() < sizeof(struct ofp_oxm_experimenter_header) + 2*sizeof(uint8_t))
		return get_u8value();
	struct rofl::openflow::ofp_oxm_ofb_exp_uint8_t* oxm =
			(struct rofl::openflow::ofp_oxm_ofb_exp_uint8_t*)somem();
