...
public:
	...
	operator<< (std::ostream& os, cofgroup_features_stats_reply const& reply) {
		...
		os << "<types: 0x" << std::hex << (int)reply.get_types() << std::dec << " >" << std::endl;
		os << "<capabilities: 0x" << std::hex << (int)reply.get_capabilities() << std::dec << " >" << std::endl;
		for (unsigned int i = 0; i < reply.max_groups.size(); ++i) {
			os << "<max-groups[" << i << "]: " << (int)reply.max_groups[i] << " >" << std::endl;
		}
		for (unsigned int i = 0; i < reply.actions.size(); ++i) {
			os << "<actions[" << i << "]: 0x" << std::hex << (int)reply.max_groups[i] << std::dec << " >" << std::endl;
		}
		return os;
	};
