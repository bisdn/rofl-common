...
size_t coftables::length() const {
  size_t len = 0;
  for (std::map<uint8_t, coftable_features>::const_iterator it = tables.begin();
       it != tables.end(); ++it) {
...
void coftables::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen)) {
    return;
  }
...
void coftables::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen)) {
    return;
  }
  ...
  tables.clear();
  while (buflen > 0) {
    struct rofl::openflow13::ofp_table_features *table =
        (struct rofl::openflow13::ofp_table_features *)buf;

    if ((be16toh(table->length) > buflen) ||
...
/*static*/ void coftables::map_tablestatsarray_to_tables(
  ...
  for (auto table_id : tablestatsarray.keys()) {
    ...
    map_match_to_prop_oxm(table_stats.get_match(),
                          table.set_properties().set_tfp_match());

    // set supported wildcards
    map_match_to_prop_oxm(table_stats.get_wildcards(),
                          table.set_properties().set_tfp_wildcards());

    // set supported write actions
    map_actions_to_prop_actions(table_stats.get_write_actions(),
                                table.set_properties().set_tfp_write_actions());
    map_actions_to_prop_actions(
        table_stats.get_write_actions(),
        table.set_properties().set_tfp_write_actions_miss());

    // set supported apply actions
    map_actions_to_prop_actions(table_stats.get_apply_actions(),
                                table.set_properties().set_tfp_apply_actions());
    map_actions_to_prop_actions(
        table_stats.get_apply_actions(),
        table.set_properties().set_tfp_apply_actions_miss());

    // set supported write setfields
    map_match_to_prop_oxm(table_stats.get_write_setfields(),
                          table.set_properties().set_tfp_write_setfield());
    map_match_to_prop_oxm(table_stats.get_write_setfields(),
                          table.set_properties().set_tfp_write_setfield_miss());

    // set supported apply setfields
    map_match_to_prop_oxm(table_stats.get_apply_setfields(),
                          table.set_properties().set_tfp_apply_setfield());
    map_match_to_prop_oxm(table_stats.get_apply_setfields(),
                          table.set_properties().set_tfp_apply_setfield_miss());

    // set instructions
    map_instructions_to_prop_instructions(
        table_stats.get_instructions(),
        table.set_properties().set_tfp_instructions());
    map_instructions_to_prop_instructions(
        table_stats.get_instructions(),
        table.set_properties().set_tfp_instructions_miss());
  }
}

/*static*/ void coftables::map_tables_to_tablestatsarray(
    rofl::openflow::coftables &tables,
    rofl::openflow::coftablestatsarray &tablestatsarray) {
  uint64_t match = 0;
  uint32_t actions = 0;
  ...
  for (std::map<uint8_t, rofl::openflow::coftable_features>::iterator it =
           tables.tables.begin();
    ...
    // add or reset table
    rofl::openflow::coftable_stats_reply &table =
        tablestatsarray.add_table_stats(table_id);
    ...
    // set supported matches
    map_prop_oxm_to_match(table_features.get_properties().get_tfp_match(),
                          match);
    table.set_match(match);

    // set supported wildcards
    map_prop_oxm_to_match(table_features.set_properties().set_tfp_wildcards(),
                          match);
    table.set_wildcards(match);

    // set supported write-actions
    map_prop_actions_to_actions(
        table_features.get_properties().get_tfp_write_actions(), actions);
    table.set_write_actions(actions);

    // set supported apply-actions
    map_prop_actions_to_actions(
        table_features.get_properties().get_tfp_apply_actions(), actions);
    table.set_apply_actions(actions);

    // set supported write-setfields
    map_prop_oxm_to_match(
        table_features.set_properties().get_tfp_write_setfield(), match);
    table.set_write_setfields(match);

    // set supported apply-setfields
    map_prop_oxm_to_match(
        table_features.set_properties().get_tfp_apply_setfield(), match);
    table.set_apply_setfields(match);

    // set supported instructions
    map_prop_instructions_to_instructions(
        table_features.get_properties().get_tfp_instructions(), instructions);
    table.set_instructions(instructions);
  }
}

/*static*/ void coftables::map_match_to_prop_oxm(
...
/*static*/ void coftables::map_prop_oxm_to_match(
    const rofl::openflow::coftable_feature_prop_oxm &prop_oxm,
    uint64_t &match) {
  match = 0;

  for (std::vector<uint32_t>::const_iterator it =
           prop_oxm.get_oxm_ids().begin();
       it != prop_oxm.get_oxm_ids().end(); ++it) {
    switch (*it) {
...
/*static*/ void coftables::map_actions_to_prop_actions(
    uint32_t actions,
...
/*static*/ void coftables::map_prop_actions_to_actions(
  ...
  for (std::vector<struct rofl::openflow::ofp_action>::const_iterator it =
           prop_actions.get_action_ids().begin();
       it != prop_actions.get_action_ids().end(); ++it) {
    switch (it->type) {
...
/*static*/ void coftables::map_instructions_to_prop_instructions(
  ...
  if (instructions &
      (((uint64_t)1) << rofl::openflow12::OFPIT_WRITE_METADATA)) {
...
/*static*/ void coftables::map_prop_instructions_to_instructions(
    const rofl::openflow::coftable_feature_prop_instructions &prop_instructions,
    uint32_t &instructions) {
  instructions = 0;

  for (std::vector<struct rofl::openflow::ofp_instruction>::const_iterator it =
           prop_instructions.get_instruction_ids().begin();
       it != prop_instructions.get_instruction_ids().end(); ++it) {
    switch (it->type) {
