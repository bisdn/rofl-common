...
public:
   ...
   *
   */
  cofflowstatsarray(const cofflowstatsarray &array) { *this = array; };

  /**
   *
   */
  cofflowstatsarray &operator=(const cofflowstatsarray &array) {
    if (this == &array)
      return *this;
   ...
   *
   */
  bool operator==(const cofflowstatsarray &flows) const {
   ...
   *
   */
  cofflowstatsarray &operator+=(const cofflowstatsarray &flows) {
    uint32_t flow_id = 0;
...
public:
   ...
   *
   */
  cofflowstatsarray &set_version(uint8_t ofp_version) {
...
public:
   ...
   *
   */
  cofflow_stats_reply &add_flow_stats() {
    uint32_t flow_id = 0;
    AcquireReadWriteLock rwlock(array_lock);
   ...
   *
   */
  const cofflow_stats_reply &get_flow_stats(uint32_t flow_id) const {
    AcquireReadLock rwlock(array_lock);
    if (array.find(flow_id) == array.end()) {
   ...
   *
   */
  bool drop_flow_stats(uint32_t flow_id) {
    AcquireReadWriteLock rwlock(array_lock);
    if (array.find(flow_id) == array.end()) {
      return false;
    }
   ...
   *
   */
  bool has_flow_stats(uint32_t flow_id) const {
    AcquireReadLock rwlock(array_lock);
...
public:
  friend std::ostream &operator<<(std::ostream &os,
    ...
    os << "<cofflowstatsarray #flows:" << (int)flowstatsarray.array.size()
       << " >" << std::endl;

    for (std::map<uint32_t, cofflow_stats_reply>::const_iterator it =
             flowstatsarray.array.begin();
         it != flowstatsarray.array.end(); ++it) {
      os << it->second;
    }
    return os;
  }
