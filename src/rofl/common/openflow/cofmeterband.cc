...
cofmeter_band::cofmeter_band(uint8_t of_version, size_t len)
    : of_version(of_version), type(0), len(len), rate(0), burst_size(0),
      body(len - sizeof(struct rofl::openflow13::ofp_meter_band_header)) {}

cofmeter_band::~cofmeter_band() {}

cofmeter_band::cofmeter_band(cofmeter_band const &mb) { *this = mb; }

cofmeter_band &cofmeter_band::operator=(cofmeter_band const &mb) {
  if (this == &mb)
    return *this;
...
size_t cofmeter_band::length() const {
  switch (of_version) {
  case rofl::openflow13::OFP_VERSION: {
    return (sizeof(struct rofl::openflow13::ofp_meter_band_header) +
            body.memlen());
  } break;
  default:
...
void cofmeter_band::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
...
void cofmeter_band::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
...
cofmeter_band_drop::cofmeter_band_drop(uint8_t of_version)
    : cofmeter_band(of_version,
                    sizeof(struct rofl::openflow13::ofp_meter_band_drop)) {
  switch (of_version) {
  case rofl::openflow13::OFP_VERSION: {
...
cofmeter_band_drop::cofmeter_band_drop(const cofmeter_band_drop &mb)
    : cofmeter_band(mb.get_version()) {
  *this = mb;
}

cofmeter_band_drop &cofmeter_band_drop::
operator=(const cofmeter_band_drop &mb) {
  if (this == &mb)
    return *this;
...
cofmeter_band_dscp_remark::cofmeter_band_dscp_remark(uint8_t of_version)
    : cofmeter_band(of_version,
                    sizeof(struct rofl::openflow13::ofp_meter_band_header)),
      prec_level(0) {
  switch (of_version) {
...
size_t cofmeter_band_dscp_remark::length() const {
  switch (get_version()) {
  case rofl::openflow13::OFP_VERSION: {
...
void cofmeter_band_dscp_remark::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
...
void cofmeter_band_dscp_remark::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
...
cofmeter_band_experimenter::cofmeter_band_experimenter(uint8_t of_version)
    : cofmeter_band(
          of_version,
          sizeof(struct rofl::openflow13::ofp_meter_band_header)), // yes, do
                                                                   // not store
                                                                   // exp_id
                                                                   // here
      exp_id(0) {
...
size_t cofmeter_band_experimenter::length() const {
  switch (get_version()) {
  case openflow13::OFP_VERSION: {
    return (sizeof(struct openflow13::ofp_meter_band_experimenter) +
            exp_body.memlen());
  } break;
  default: {
    throw eBadVersion("eBadVersion", __FILE__, __PRETTY_FUNCTION__, __LINE__);
...
void cofmeter_band_experimenter::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
...
void cofmeter_band_experimenter::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
  ...
  case rofl::openflow13::OFP_VERSION: {
    ...
    if (buflen > sizeof(struct openflow13::ofp_meter_band_experimenter)) {
      exp_body.assign(
          mbh->body,
          buflen - sizeof(struct openflow13::ofp_meter_band_experimenter));
    }
  } break;
  default: {
    throw eBadVersion("eBadVersion", __FILE__, __PRETTY_FUNCTION__, __LINE__);
