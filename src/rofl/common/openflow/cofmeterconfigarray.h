...
public:
   ...
   *
   */
  cofmeterconfigarray(const cofmeterconfigarray &mconfig) { *this = mconfig; };
...
public:
   ...
   *
   */
  bool drop_meter_config(unsigned int index = 0) {
    AcquireReadWriteLock lock(array_lock);
    if (array.find(index) == array.end()) {
      return false;
    }
...
public:
  friend std::ostream &operator<<(std::ostream &os,
                                  cofmeterconfigarray const &mconfig) {
    os << "<cofmeterconfigarray #meter-config:" << (int)mconfig.array.size()
       << " >" << std::endl;
