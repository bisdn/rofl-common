...
cofpacket_queues::cofpacket_queues(uint8_t ofp_version)
    : ofp_version(ofp_version) {}

cofpacket_queues::~cofpacket_queues() {}

cofpacket_queues::cofpacket_queues(const cofpacket_queues &queues) {
  *this = queues;
}

cofpacket_queues &cofpacket_queues::operator=(const cofpacket_queues &queues) {
  if (this == &queues)
    return *this;
  ...
  for (std::map<uint32_t, std::map<uint32_t, cofpacket_queue>>::const_iterator
           it = queues.get_packet_queues().begin();
       it != queues.get_packet_queues().end(); ++it) {
...
void cofpacket_queues::clear() { pqueues.clear(); }

cofpacket_queue &cofpacket_queues::add_pqueue(uint32_t port_no,
                                              uint32_t queue_id) {
...
cofpacket_queue &cofpacket_queues::set_pqueue(uint32_t port_no,
                                              uint32_t queue_id) {
  if (pqueues[port_no].find(queue_id) == pqueues[port_no].end()) {
    pqueues[port_no][queue_id] =
        cofpacket_queue(ofp_version, port_no, queue_id);
  }
  return pqueues[port_no][queue_id];
}

const cofpacket_queue &cofpacket_queues::get_pqueue(uint32_t port_no,
...
void cofpacket_queues::drop_pqueue(uint32_t port_no, uint32_t queue_id) {
  if (pqueues[port_no].find(queue_id) == pqueues[port_no].end()) {
    return;
  }
  pqueues[port_no].erase(queue_id);
...
bool cofpacket_queues::has_pqueue(uint32_t port_no, uint32_t queue_id) const {
  return (not(pqueues.at(port_no).find(queue_id) == pqueues.at(port_no).end()));
}

size_t cofpacket_queues::length() const {
  size_t len = 0;
  for (std::map<uint32_t, std::map<uint32_t, cofpacket_queue>>::const_iterator
           it = pqueues.begin();
       it != pqueues.end(); ++it) {
...
void cofpacket_queues::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
  ...
  case rofl::openflow13::OFP_VERSION: {
    ...
    for (std::map<uint32_t, std::map<uint32_t, cofpacket_queue>>::iterator it =
             pqueues.begin();
         it != pqueues.end(); ++it) {
      for (std::map<uint32_t, cofpacket_queue>::iterator jt =
               it->second.begin();
           jt != it->second.end(); ++jt) {
...
void cofpacket_queues::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
