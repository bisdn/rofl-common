...
/*static*/ int64_t cofrole::distance(int64_t rcvd_gen_id,
                                     int64_t cached_gen_id) {
  return (rcvd_gen_id - cached_gen_id);
}

cofrole::cofrole(uint8_t ofp_version)
    : ofp_version(ofp_version), role(rofl::openflow13::OFPCR_ROLE_EQUAL),
      generation_id((int64_t)-1) {}

cofrole::~cofrole() {}

cofrole::cofrole(cofrole const &role) { *this = role; }

cofrole &cofrole::operator=(cofrole const &r) {
  if (this == &r)
    return *this;
...
void cofrole::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
...
void cofrole::unpack(uint8_t *buf, size_t buflen) {
  if (buflen < length())
    throw eInvalid("eInvalid", __FILE__, __PRETTY_FUNCTION__, __LINE__);
...
bool cofrole::is_valid_role() const {
  switch (get_role()) {
