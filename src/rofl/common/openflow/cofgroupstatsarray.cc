...
void cofgroupstatsarray::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
  ...
  case rofl::openflow13::OFP_VERSION: {
    ...
    for (std::map<uint32_t, cofgroup_stats_reply>::iterator it = array.begin();
         it != array.end(); ++it) {
...
void cofgroupstatsarray::unpack(uint8_t *buf, size_t buflen) {
  array.clear();
  ...
  case rofl::openflow12::OFP_VERSION: {
    ...
    while (buflen >= sizeof(struct rofl::openflow12::ofp_group_stats)) {
      ...
      size_t length =
          be16toh(((struct rofl::openflow12::ofp_group_stats *)buf)->length);

      if (length < sizeof(struct rofl::openflow12::ofp_group_stats))
        return;

      uint32_t group_id =
          be32toh(((struct rofl::openflow12::ofp_group_stats *)buf)->group_id);
  ...
  case rofl::openflow13::OFP_VERSION: {
    ...
    while (buflen >= sizeof(struct rofl::openflow13::ofp_group_stats)) {
      ...
      size_t length =
          be16toh(((struct rofl::openflow13::ofp_group_stats *)buf)->length);

      if (length < sizeof(struct rofl::openflow13::ofp_group_stats))
        return;

      uint32_t group_id =
          be32toh(((struct rofl::openflow13::ofp_group_stats *)buf)->group_id);
