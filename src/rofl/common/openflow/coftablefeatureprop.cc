...
coftable_feature_prop::coftable_feature_prop(uint8_t ofp_version, uint16_t type,
                                             size_t len)
...
coftable_feature_prop &coftable_feature_prop::
operator=(coftable_feature_prop const &tfp) {
  if (this == &tfp)
    return *this;
...
size_t coftable_feature_prop::length() const { return cmemory::memlen(); }

void coftable_feature_prop::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
...
void coftable_feature_prop::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen)) {
    throw eOFTableFeaturePropInval();
  }
...
uint8_t coftable_feature_prop::get_version() const { return ofp_version; }

uint8_t *coftable_feature_prop::resize(size_t size) {
  return (ofp_tfp = cmemory::resize(size));
}

uint16_t coftable_feature_prop::get_type() const {
  return be16toh(ofp_tfphdr->type);
}

void coftable_feature_prop::set_type(uint16_t type) {
  ofp_tfphdr->type = htobe16(type);
}

uint16_t coftable_feature_prop::get_length() const {
  return be16toh(ofp_tfphdr->length);
}

void coftable_feature_prop::set_length(uint16_t len) {
...
coftable_feature_prop_instructions::coftable_feature_prop_instructions(
    ...
    : coftable_feature_prop(
          ofp_version, type,
          sizeof(struct openflow13::ofp_table_feature_prop_instructions)) {}
...
operator=(coftable_feature_prop_instructions const &tfpi) {
  ...
  // std::copy(tfpi.instructions_ids.begin(), tfpi.instructions_ids.end(),
  // instructions_ids.begin());

  return *this;
}

void coftable_feature_prop_instructions::clear() {
  instructions_ids.clear();
  resize(sizeof(struct rofl::openflow13::ofp_table_feature_prop_instructions));
  set_length(
      sizeof(struct rofl::openflow13::ofp_table_feature_prop_instructions));
}

size_t coftable_feature_prop_instructions::length() const {
  // TODO: support for experimental instructions
  size_t total_length =
      sizeof(struct openflow13::ofp_table_feature_prop_header) +
...
void coftable_feature_prop_instructions::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen)) {
    return;
  }
  ...
  set_length(sizeof(struct openflow13::ofp_table_feature_prop_header) +
             instructions_ids.size() *
                 sizeof(struct openflow::ofp_instruction)); // without padding
  ...
  for (unsigned int i = 0; i < instructions_ids.size(); i++) {
    struct rofl::openflow::ofp_instruction *inst =
...
void coftable_feature_prop_instructions::unpack(uint8_t *buf, size_t buflen) {
  try {
    if ((0 == buf) ||
        (buflen <
         sizeof(struct openflow13::ofp_table_feature_prop_instructions))) {
    ...
    // sanity check: overall buflen must contain length field from tfpihdr +
    // padding
...
uint8_t *coftable_feature_prop_instructions::resize(size_t size) {
  return (ofh_tfpi = coftable_feature_prop::resize(size));
}

void coftable_feature_prop_instructions::add_instruction(uint16_t type,
                                                         uint16_t len) {
  struct rofl::openflow::ofp_instruction instruction;
...
coftable_feature_prop_next_tables::coftable_feature_prop_next_tables(
    ...
    : coftable_feature_prop(
          ofp_version, type,
          sizeof(struct openflow13::ofp_table_feature_prop_next_tables)) {}
...
coftable_feature_prop_next_tables &coftable_feature_prop_next_tables::
operator=(coftable_feature_prop_next_tables const &tfpnxt) {
  if (this == &tfpnxt)
    return *this;
  ...
  for (std::vector<uint8_t>::const_iterator it = tfpnxt.begin();
       it != tfpnxt.end(); ++it) {
    (*this).push_back(*it);
  }
...
void coftable_feature_prop_next_tables::clear() {
  std::vector<uint8_t>::clear();
  resize(sizeof(struct rofl::openflow13::ofp_table_feature_prop_next_tables));
  set_length(
      sizeof(struct rofl::openflow13::ofp_table_feature_prop_next_tables));
}

size_t coftable_feature_prop_next_tables::length() const {
  size_t total_length =
      sizeof(struct openflow13::ofp_table_feature_prop_next_tables) +
...
void coftable_feature_prop_next_tables::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen)) {
    return;
  }
...
void coftable_feature_prop_next_tables::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) ||
      (buflen <
       sizeof(struct openflow13::ofp_table_feature_prop_next_tables))) {
    throw eOFTableFeaturePropInval();
  ...
  // sanity check: overall buflen must contain length field from tfpnxthdr +
  // padding
  size_t total_length = get_length();
  size_t pad = (0x7 & total_length);
  /* append padding if not a multiple of 8 */
...
uint8_t *coftable_feature_prop_next_tables::resize(size_t size) {
  return (ofh_tfpnxt = coftable_feature_prop::resize(size));
}

void coftable_feature_prop_next_tables::add_table_id(uint8_t table_id) {
...
coftable_feature_prop_actions::coftable_feature_prop_actions(
    ...
    : coftable_feature_prop(
          ofp_version, type,
          sizeof(struct openflow13::ofp_table_feature_prop_actions)) {}
...
void coftable_feature_prop_actions::clear() {
  actions.clear();
...
size_t coftable_feature_prop_actions::length() const {
...
void coftable_feature_prop_actions::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen)) {
    return;
  }
  ...
  set_length(sizeof(struct rofl::openflow13::ofp_table_feature_prop_header) +
             actions.size() *
                 sizeof(struct openflow::ofp_action)); // without padding

  memcpy(buf, somem(),
         sizeof(struct rofl::openflow13::ofp_table_feature_prop_header));

  buf += sizeof(struct rofl::openflow13::ofp_table_feature_prop_header);

  // fill in next-tables-ids (internal buffer)
  for (std::vector<struct rofl::openflow::ofp_action>::iterator it =
           actions.begin();
       it != actions.end(); ++it) {
...
void coftable_feature_prop_actions::unpack(uint8_t *buf, size_t buflen) {
  try {
    if ((0 == buf) ||
        (buflen < sizeof(struct openflow13::ofp_table_feature_prop_actions))) {
      throw eOFTableFeaturePropInval();
    }
    ...
    for (unsigned int i = 0; i < n_action_ids; i++) {
      struct rofl::openflow::ofp_action *act =
          (struct rofl::openflow::ofp_action *)buf;
      struct rofl::openflow::ofp_action action;
...
uint8_t *coftable_feature_prop_actions::resize(size_t size) {
  return (ofh_tfpa = coftable_feature_prop::resize(size));
}

void coftable_feature_prop_actions::add_action(uint16_t type, uint16_t len) {
  struct rofl::openflow::ofp_action action;
  action.type = type;
  action.len = len;
  actions.push_back(action);
...
coftable_feature_prop_oxm::coftable_feature_prop_oxm(uint8_t ofp_version,
                                                     uint16_t type)
    : coftable_feature_prop(
          ofp_version, type,
          sizeof(struct openflow13::ofp_table_feature_prop_oxm)) {}
...
coftable_feature_prop_oxm &coftable_feature_prop_oxm::
operator=(coftable_feature_prop_oxm const &tfpoxm) {
  if (this == &tfpoxm)
    return *this;
  ...
  oxm_ids.clear();
  for (std::vector<uint32_t>::const_iterator it = tfpoxm.oxm_ids.begin();
       it != tfpoxm.oxm_ids.end(); ++it) {
    oxm_ids.push_back(*it);
  }
  // std::copy(tfpoxm.oxm_ids.begin(), tfpoxm.oxm_ids.end(), oxm_ids.begin());
  oxm_ids_exp.clear();
  for (std::vector<uint64_t>::const_iterator it = tfpoxm.oxm_ids_exp.begin();
       it != tfpoxm.oxm_ids_exp.end(); ++it) {
    oxm_ids_exp.push_back(*it);
  }
  // std::copy(tfpoxm.oxm_ids_exp.begin(), tfpoxm.oxm_ids_exp.end(),
  // oxm_ids_exp.begin());

  return *this;
}

void coftable_feature_prop_oxm::clear() {
  oxm_ids.clear();
  oxm_ids_exp.clear();
...
size_t coftable_feature_prop_oxm::length() const {
  size_t total_length = sizeof(struct openflow13::ofp_table_feature_prop_oxm) +
                        oxm_ids.size() * sizeof(uint32_t) +
                        oxm_ids_exp.size() * sizeof(uint64_t);
...
void coftable_feature_prop_oxm::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen)) {
    return;
  }
  ...
  set_length(sizeof(struct rofl::openflow13::ofp_table_feature_prop_oxm) +
             oxm_ids.size() * sizeof(uint32_t) +
             oxm_ids_exp.size() * sizeof(uint64_t)); // without padding

  memcpy(buf, somem(),
         sizeof(struct rofl::openflow13::ofp_table_feature_prop_oxm));

  // fill in oxm-ids (internal buffer)
  uint32_t *uint32 = (uint32_t *)((
      uint8_t *)(buf +
                 sizeof(struct rofl::openflow13::ofp_table_feature_prop_oxm)));
  ...
  uint64_t *uint64 =
      (uint64_t *)((uint8_t *)(buf + sizeof(struct rofl::openflow13::
...
void coftable_feature_prop_oxm::unpack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) ||
      (buflen < sizeof(struct openflow13::ofp_table_feature_prop_oxm))) {
    throw eOFTableFeaturePropInval();
  }
  ...
  // sanity check: overall buflen must contain length field from tfpoxmhdr +
  // padding
  size_t total_length = get_length();
  size_t pad = (0x7 & total_length);
  /* append padding if not a multiple of 8 */
  ...
  size_t remaining =
      get_length() - sizeof(struct openflow13::ofp_table_feature_prop_oxm);
  while (remaining >= sizeof(uint32_t)) {
    struct openflow::ofp_oxm_hdr *oxm =
        (struct openflow::ofp_oxm_hdr
             *)(((struct openflow13::ofp_table_feature_prop_oxm *)buf)
                    ->oxm_ids);
...
uint8_t *coftable_feature_prop_oxm::resize(size_t size) {
  return (ofh_tfpoxm = coftable_feature_prop::resize(size));
}

void coftable_feature_prop_oxm::add_oxm(uint32_t oxm_id) {
  oxm_ids.push_back(oxm_id);
}

void coftable_feature_prop_oxm::add_oxm_exp(uint64_t oxm_exp_id) {
