...
size_t cofbucket_counters::length() const {
  size_t len = 0;
  for (std::map<uint32_t, cofbucket_counter>::const_iterator it =
           bucketcounters.begin();
       it != bucketcounters.end(); ++it) {
    len += it->second.length();
  }
  return len;
}

void cofbucket_counters::pack(uint8_t *buf, size_t buflen) {
  if ((0 == buf) || (0 == buflen))
    return;
  ...
  case rofl::openflow13::OFP_VERSION: {
    for (std::map<uint32_t, cofbucket_counter>::iterator it =
             bucketcounters.begin();
         it != bucketcounters.end(); ++it) {
...
void cofbucket_counters::unpack(uint8_t *buf, size_t buflen) {
  clear();
  ...
  case rofl::openflow12::OFP_VERSION: {
    ...
    while (buflen >= sizeof(struct rofl::openflow12::ofp_bucket_counter)) {
      add_bucket_counter(bucket_counter_id++)
