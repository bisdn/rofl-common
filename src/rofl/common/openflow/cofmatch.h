...
public:
	...
	uint16_t get_tp_dst() const {
	 return static_cast<const coxmatch_ofx_tp_dst*>(matches.get_match(rofl::openflow::experimental::OXM_TLV_EXPR_TP_DST))->get_u16value();
	};

	// OF12
	uint32_t get_in_port() const {
	 return static_cast<const coxmatch_ofb_in_port*>(matches.get_match(OXM_TLV_BASIC_IN_PORT))->get_u32value();
	};
