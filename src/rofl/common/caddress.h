...
public:
   ...
   *
   */
  caddress(size_t size = 0) : rofl::cmemory(size){};

  /**
   *
   */
  caddress(const rofl::cmemory &addr) : rofl::cmemory(addr){};
  ...
  caddress operator&(const caddress &addr) const {
    return caddress(cmemory::operator&(addr));
  };
...
private:
   ...
   */
  virtual uint8_t *resize(size_t size) { return cmemory::resize(size); };
...
public:
   ...
   *
   */
  caddress_ll() : caddress(ETH_ADDR_LEN){};

  /**
   *
   */
  caddress_ll(uint8_t *buf, size_t buflen) : caddress(ETH_ADDR_LEN) {
   ...
   *
   */
  caddress_ll(const std::string &addr) : caddress(ETH_ADDR_LEN) {
    str2addr(addr);
  };
   ...
   *
   */
  caddress_ll(uint64_t mac) : caddress(ETH_ADDR_LEN) { set_mac(mac); };
   ...
   *
   */
  caddress_ll &operator=(const caddress_ll &addr) {
    if (this == &addr)
      return *this;
...
public:
  ...
  /**
   * @brief	Return the MAC address as a uint64_t value. The MAC address will
   * be transformed to host byte order and resides in the lower bytes.
   *
   * @return uint64_t mac
   */
...
private:
   ...
   *
   */
  void str2addr(const std::string &addr);

  /**
   *
   */
  std::string addr2str() const;
...
public:
   ...
   *
   */
  caddress_in(size_t size = 0) : caddress(size){};
   ...
   *
   */
  caddress_in &operator=(const caddress_in &addr) {
    if (this == &addr)
      return *this;
    caddress::operator=(addr);
    return *this;
   ...
   *
   */
  bool operator<(const caddress_in &addr) const {
    return caddress::operator<(addr);
  };
...
public:
  friend std::ostream &operator<<(std::ostream &os, const caddress_in &addr) {
...
public:
   ...
   *
   */
  caddress_in4() : caddress_in(INET4_ADDR_LEN){};

  /**
   *
   */
  caddress_in4(const std::string &addr) : caddress_in(INET4_ADDR_LEN) {
    str2addr(addr);
  };
   ...
   *
   */
  caddress_in4(struct sockaddr_in *sin, socklen_t salen)
   ...
   *
   */
  caddress_in4 &operator=(const caddress_in4 &addr) {
    if (this == &addr)
      return *this;
   ...
   *
   */
  bool operator<(const caddress_in4 &addr) const {
   ...
   *
   */
  caddress_in4 operator&(const caddress_in4 &addr) const {
...
public:
   ...
   *
   */
  caddress_in6() : caddress_in(INET6_ADDR_LEN){};

  /**
   *
   */
  caddress_in6(const std::string &addr) : caddress_in(INET6_ADDR_LEN) {
    str2addr(addr);
  };
   ...
   *
   */
  caddress_in6(struct sockaddr_in6 *sin6, socklen_t salen)
   ...
   *
   */
  caddress_in6 &operator=(const caddress_in6 &addr) {
    if (this == &addr)
      return *this;
    caddress_in::operator=(addr);
    return *this;
   ...
   *
   */
  bool operator<(const caddress_in6 &addr) const {
    return caddress_in::operator<(addr);
  };
   ...
   *
   */
  caddress_in6 operator&(const caddress_in6 &addr) const {
