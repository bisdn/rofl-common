...
caddrinfo &caddrinfos::add_addr_info(unsigned int index) {
...
caddrinfo &caddrinfos::set_addr_info(unsigned int index) { return ais[index]; }

void caddrinfos::drop_addr_info(unsigned int index) {
  if (ais.find(index) == ais.end()) {
    return;
  }
  ais.erase(index);
...
const caddrinfo &caddrinfos::get_addr_info(unsigned int index) const {
  if (ais.find(index) == ais.end()) {
...
bool caddrinfos::has_addr_info(unsigned int index) const {
