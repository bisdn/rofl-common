...
public:
  eMemBase(const std::string &__arg) : exception(__arg){};
};
class eMemNotFound : public eMemBase {
public:
  eMemNotFound(const std::string &__arg) : eMemBase(__arg){};
};
...
private:
  ...
  std::pair<uint8_t *, size_t>
      data; //< memory area including head- and tail-space

#define CMEMORY_DEFAULT_SIZE 0

public:
  /**
   * @brief	Constructor. Allocates a new memory area with specified size
   * (default: 1024 bytes).
   *
  ...
  /**
   * @brief	Constructor. Allocates a new memory area and clones specified
   * buffer.
   *
...
public:
   ...
   */
  cmemory &operator=(const cmemory &m);
   ...
   * @exception eMemOutOfRange thrown when index points beyond end of allocated
   * memory area
   */
  ...
  /**
   * @brief	Overwrites memory area with plain buffer specified. Resizes
   * internal memory as necessary.
   *
   ...
   *
   */
  virtual size_t length() const { return data.second; };
   ...
   * @exception eMemInval is thrown when ptr points to a location outside of the
   ...
   * @param len number of bytes inserted in the allocated memory area
   * @exception eMemInval is thrown when offset refers to a location outside of
   * the allocated memory area
   */
  uint8_t *insert(unsigned int offset, size_t len);

  /**
   * @brief	Removes len bytes starting at pointer ptr from allocated memory
   * area.
   *
  ...
  /**
   * @brief	Removes len bytes starting at offset relative to the allocated
   * memory area.
   *
   * @param offset relative start location of memory block to be removed
   * @param len length of memory block to be removed
   * @exception eMemInval is thrown when the relative start location points to a
   * location outside of the allocated memory area
   */
  void remove(unsigned int offset, size_t len);

  /**
   * @brief	Returns index of first byte with value "value" found in
   * allocated
   * memory area starting at offset "start".
   *
   * @param value sought value
   * @param start relative begin within allocated memory area for search
   * @exception eMemNotFound is thrown when a byte with the specified value
   * cannot be found
   */
   ...
   */
  virtual void unpack(uint8_t *buf, size_t buflen) { assign(buf, buflen); };
   ...
   */
  virtual bool empty() const { return (0 == memlen()); };
...
public:
  friend std::ostream &operator<<(std::ostream &os, const cmemory &mem) {
    os << "<cmemory: data:" << (void *)mem.data.first
       << " datalen:" << (int)mem.data.second << ">" << std::endl;
    ...
    if (mem.data.second > 0) {
      for (unsigned int i = 0; i < mem.data.second; i++) {
        if (0 == (i % width)) {
          os << std::setfill('0') << std::setw(4) << std::dec << (i / width)
             << ": " << std::hex << std::setw(0) << std::setfill(' ');
        }

        os << std::setfill('0') << std::setw(2) << std::hex
           << (int)(*(mem.somem() + i)) << std::dec << std::setw(0)
           << std::setfill(' ') << " ";
